...
>
> **实施状态（2026-07-28）**：第一批 A1~A4、第二批 B5~B7、第三批 B8~B9 已完成代码实现与程序化测试；
> **第四批已按用户反馈重排为「持仓卖出决策提醒」专项**（见 §6），原 C10~C13 顺延。
...
| 四 | 成本止盈止损触发与 fail-closed、持仓期峰值更新与回撤判定、利空事件→卖出复核映射、AI 卖出建议枚举与核验、待办过滤后的分类计数 | 真实持仓上的卖出提醒实测、待办改造后的观感 |
