...
推荐算法将 `score` 展示值与六位小数的 `ranking_score` 分开。`ps1` 在全部策略命中上先做本地预选，来源轮转分配预算；`sq1` 表达突破/回踩质量，`qr1` 分组评分，`rt1` 区分收盘信号与现价，`ep3` 在 AI 输出之后重新取得报价核验执行条件。完整 `of1` 候选事实保存 SHA-256，历史 `fv6` 因子快照直接保存新特征，不从今天的价格反算旧特征。

`ranking_model_artifacts` 保存不可变模型与时间外证据；`ranking_scoring_policies` 按推荐周期和评分侧重选择 `qr1/additive_sp1/ridge1`，`ranking_policy_audits` 追加切换历史。学习模型必须先研究、保存，满足条件后再显式启用；样本不足不能自动训练或转正。完整算法、只读评估口径和 CLI 用法见 [推荐排序与评估](RECOMMENDATION_RANKING.md)。
