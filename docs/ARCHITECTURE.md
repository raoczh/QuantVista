...
用户发起推荐（或收盘日报 GenerateAuto 同链路）
  |
1. 多源建池：自选 ∪ 涨幅榜 ∪ 成交额榜 ∪ 换手率榜（每源 20，来源可叠加落库；黑名单/ST/流动性/ETF基金 前置排除）
  |
2. 用户筛选硬过滤：股价/流通市值/换手率区间、排除当日涨停、近5日追高保护（20cm 板放大）、换手>20% 死亡换手硬拦
  |     （被筛掉的标的保留在池快照并标注原因；条件快照 filters_json 落库可回显）
  |
3. 本地量化评分（零 LLM 成本）：Top36 拉 90 根日线算技术因子 + 五维评分 + 策略加分，池内排名
  |
4. LLM 精选：只见量化 Top12，强制引用字段名+数值、禁先验记忆、允许少选甚至 0 只；越池/非 Top12 标的一律丢弃
  |
校验结构化输出（有限次 repair）→ 信任层回填（见 §6.8）→ 批次+条目事务落库 → 初始化追踪
```

关键常量：`maxScanCandidates=36 / maxLLMCandidates=12 / maxPoolIntake=200 / poolSnapshotMax=150`。
