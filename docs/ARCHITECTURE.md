...
- **Tushare 分档接入**：第一阶段以东财 + 新浪为主，Tushare 非前置；免费档（120，股票清单/日线/交易日历）与低 cost 档（2000，财务三表/复权因子/指数日线，长线财务深度来源）按需启用，高级档（5000，分钟线/融资融券明细等）暂不实现。详见 [数据源选型](DATA_SOURCES.md)。
...
- 页面运行时，AppShell 发起最长 25 秒长轮询；事件事务提交后唤醒本进程，跨进程每 5 秒复查。权限状态随授权、设置变化和重新聚焦刷新，Worker 激活后再订阅。权限请求只由设置页按钮触发。
- 在线投递和 Web Push 共用 `/sw.js` 的展示入口，以用户/事件编号、系统 tag 和 IndexedDB 去重；展示成功后确认。无 Worker 时才退回页面 Notification API。VAPID 缺失时只能保证页面运行期间的接收，后台休眠和关闭页面需有效 Push 订阅。
- 服务端持久保存重试时点及 UTC 毫秒租约，防止并发重复认领；每 15 秒恢复待投递事件，临时错误退避，过期行情不补弹。推送服务接收、展示回执和业务事实分别记录，HTTP 成功不冒充 OS 已展示。
- `/sw.js` 的 `notificationclick` 只接受同源相对路径；未登录时
...
- 自定义选股策略 hash 对名称、说明、周期、风险、评分侧重和结构化规范后的完整条件树计算 SHA-256，不直接使用用户原始 JSON 文本；对象键序、空白和等价数字写法不造成 hash 漂移。旧版本空评分侧重的摘要继续兼容。
...
推荐算法将 `score` 展示值与六位小数的 `ranking_score` 分开。`ps2` 在全部策略命中上按侧重及形态意图预选，来源轮转分配预算；`sq2` 表达突破/回踩质量，连续突破使用起点前 ATR；`qr2` 分组评分，`rt1` 区分收盘信号与现价，`ep5` 在 AI 输出后重新取得报价核验执行条件。完整 `of2` 候选事实保存 SHA-256，`fv7` 因子快照直接保存新特征并要求复权质量已确认，不从今天的价格反算旧输入。

`ranking_model_artifacts` 保存不可变模型与时间外证据；`ranking_scoring_policies` 按推荐周期和评分侧重选择 `qr2/additive_sp1/ridge1`，`ranking_policy_audits` 追加切换历史。旧 qr1 政策在新请求中使用 qr2，批次冻结实际版本。研究报告为 `rr2`，学习模型必须按新特征重新研究、保存并满足条件后显式启用。完整算法、只读评估口径和 CLI 用法见 [推荐排序与评估](RECOMMENDATION_RANKING.md)。

`rp1` 为个股分析与推荐追踪共用的研究价格入口：策略、周期、报价和已完成的 120 根尾部日线形成输入摘要；突破、回踩和反转分别确定买入参照，退出复用 `xp1`。共同计划回填既有买卖价字段，AI 原始提案单独记录且不能覆盖有效计划。报价更新仅重新核对执行条件，历史 JSON 不重算。

财务 `ff1` 保留数值可用性及报告/公告日期。同比来自最新已披露报告，年度 ROE 来自最近已披露年报，两组行在预热前同时冻结。旧缓存零值视为未知，新字段明确区分 null 和真实零；部分财务缺失会阻止依赖这些字段的策略进入可执行状态。年报 ROE 同时用于质量评分与学习特征，不直接年化季度累计值。

执行结果使用 `so3`，计划标签使用 `l4`：沪深主板风险警示股按交易日期区分 2026-07-06 前的 5% 和之后的 10%，模拟入场和每个卖出日分别检查，科创板最低买入 200 股。已完成旧版本结果保留，当前比较不混用旧口径；未完成标签以本次实际执行版本结算。

推荐作业请求 v4 只记录规范化请求与运行摘要；完整策略定义、条件树和模型参数保存在本人批次 `runtime_snapshot`。初次提交在同一接受事务创建批次，重启读取该批次，重试复制原批次配置，均核对摘要与用户身份。未冻结完整算法的旧 v1～v3 未完成作业明确提示重新生成，已完成结果不改写。
