...
- **下一步：阶段 2 收尾（市场数据补全）或阶段 3（自选股/持仓）**。阶段 2 仍欠：全市场日线批量同步、完整交易日历/休市日补全、市场快照表、涨跌家数情绪、资金流。
...
> 进度：市场首页已从验证页升级为**市场看板**（指数概览/涨幅榜/热门榜/板块榜/个股速查 + 情绪/资金流/新闻/AI 占位，commit `00d927a`）。指数/榜单/个股走新浪（稳），板块走东财 clist（间歇限流，best-effort）。**已补**：个股日线查询触发写入 `daily_bars`，并从日线推导开市日写入 `trading_calendar`。**未做**：全市场日线批量同步、完整休市日历、市场快照表、涨跌家数等市场情绪、资金流。
...
- 市场快照表与市场首页接口。
- 日线 OHLC（`daily_bars`）入库，供后续追踪复用。（已完成个股查询触发入库，未做全市场批量同步）
- 交易日历（`trading_calendar`）入库。（已完成从日线推导开市日，未做完整休市日历）
- 首页图表和榜单。
- 数据更新时间展示。
- 数据源异常提示。
