...
> **2026-07-06 起后续开发按 `REFERENCE_ANALYSIS.md` 的 P0/P1/P2 分期计划推进**（StockNova 等 6 个参考项目全量源码分析+借鉴清单+上游接口速查表；新闻舆情 P0 最优先）。以下为原有储备清单，多数已并入该计划：

- 新闻情绪（→ P0 新闻舆情闭环，财联社/东财源已调研齐）、财务详情/财报日历（→ P1 东财 datacenter 网关，免 Tushare）、回测模块（→ P2 时光机）。
