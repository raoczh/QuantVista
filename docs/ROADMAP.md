...
- **行情与数据**：东财→腾讯→新浪三源主备切换；实时行情/日线（东财前复权 fqt=1）/指数/榜单（涨幅/成交额/换手率）/板块/涨跌家数/两市资金流/腾讯免费估值（PE/PB/市值/换手/量比/振幅/涨跌停价）；交易日历回填、日线批量同步、市场情绪快照。
...
- **换手 >20%（deadTurnoverPct）是无条件硬拦**，用户筛选区间必须被钳在其内（UI :max=20），否则允许的配置构成数学死局（必然空池）。
- **LLM 只见量化 Top12 子集 map**（poolBySymbol），parseAndFilterPicks 以它做反编造校验——池内但非 Top12 的标的同样会被丢弃，这是有意设计。
...
- 新闻情绪（无稳定新闻源）、财务详情/财报日历（需 Tushare 低 cost 档）、回测模块（可复用 daily_bars，工程量大）。
- 多数据源系统级切换管理端（`data_source_configs` 接读写）。
- 东财 clist 全市场扫描候选源（B 档限流，储备）。
