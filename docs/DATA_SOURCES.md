...
### 2.6 P0-3A 能力注册与运维口径（代码已实现，待部署验收）
...
以下仍属于 **P0-3B 接线清单**，本批没有路由、按钮或隐藏副作用：

1. 新增管理员 `POST` 单能力 probe 路由；provider/capability/market 必须先由注册表白名单校验，每次只允许一个有界样本，独立限流并写 `DataSyncLog` 审计。
2. 新增管理员 `POST` 人工解冷路由；只清指定三元组的 cooldown，记录管理员、原因和解冷前剩余时间，不清除历史观测。
3. 路由、权限、CSRF/限流、审计和失败契约齐备后，管理后台再接 probe/解冷按钮。禁止用有副作用的 GET 或页面刷新代替。

部署验收至少核对：注册未调用能力为 `unknown`；空响应与错误分列；冷却倒计时自动恢复；30/45/60 日查询均不触发上游；休市不进入业务覆盖分母、停牌从日线分母扣除；`null`、数组、布尔值、多对象和超过 4KiB 的正文均被拒绝，旧无 body 请求仍可用；日历预演目标数等于完整订正范围且缺行数单列；dry-run 后改变本地覆盖事实会使旧 hash 拒绝执行；同步日志只含白名单摘要，不出现 token、cookie、密钥或原始正文；至少一个暗色主题下覆盖矩阵与缺口图例不出现硬编码白底或失真颜色。
