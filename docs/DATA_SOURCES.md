...
P0-3A 同时增强现有 `GET /api/admin/data-health` 与三个补采 POST：健康报告只查本地库，以最近 30～60 个交易日为窗口，按日期索引和硬上限生成休市、停牌、缺失、部分覆盖与未知日历；`sync-bars`、`backfill-calendar`、`wide-sync` 的新 JSON body 支持有界范围与 `dry_run`，执行必须携计划 hash 并在上游调用前根据本地事实重算。完全无 body 的旧调用继续走原兼容路径。全市场 `wide-sync` 仍只具备当前应有交易日快照能力，不能冒充历史范围回补。
...
部署验收至少核对：注册未调用能力为 `unknown`；空响应与错误分列；冷却倒计时自动恢复；30/45/60 日查询均不触发上游；休市不进入业务覆盖分母、停牌从日线分母扣除；旧无 body 请求仍可用；dry-run 后改变本地覆盖事实会使旧 hash 拒绝执行；同步日志只含白名单摘要，不出现 token、cookie、密钥或原始正文。
