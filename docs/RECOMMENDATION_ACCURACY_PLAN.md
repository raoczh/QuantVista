...
> **文档状态：长期测量与发布约束。** 本文不是当前功能待办总表；S3-6C 等已实现状态与待线上验收项看 `ROADMAP.md`，全市场发现、近 5 日候选记忆、漏选/误选复盘和持仓卖出施工看 `RECOMMENDATION_DISCOVERY_AND_EXIT_PLAN.md`。S4/S5 仍须满足真实样本门槛，不能用继续堆代码替代样本验证。
>
...
> 本方案不构成投资建议。
>
> **2026-08-10 后续施工入口**：全市场日更发现、近 5 日候选记忆、每日漏选/误选复盘，以及真实持仓统一卖出决策，统一按
> [`RECOMMENDATION_DISCOVERY_AND_EXIT_PLAN.md`](./RECOMMENDATION_DISCOVERY_AND_EXIT_PLAN.md) 分批实施；本文继续作为测量口径、PIT、配对评估和 S4/S5 发布门的权威约束。
...
S3.6 数据时点一致性 + 量化 Top-N / AI 精选增量归因（先冻结事实，再做同批配对）
...
### 0.1 Top50 P1-0 U09/U26：适配与执行质量（2026-08-07）

本批把已有推荐结果进一步适配为用户可研究、可确认的执行计划，归类为**适配与执行质量**，不是模型预测精度提升：
...
**S3-6C score-blind 影子实验（2026-08-04 代码已实现，待线上部署验收）**：它是 `experiment_type=score_blind`、输入 schema `sb1` 的独立 recommendation input experiment，不是普通 prompt challenger，也不进入 prompt 的 release-audit/promote/rollback 路径；旧实验 `experiment_type` 为空时继续按 prompt challenger + `ep1` 读取。champion 与 score-blind 共用 qf4 后完全相同的候选 symbol 集合、市场上下文、模型配置、路由目标、输出 schema 和已计算业务预算；score-blind 只改输入表达：剥除 `score/rank/score_dims/bonus/strategy_notes` 及派生排序提示，保留真实原始因子、财务、资金、行情时点与缺失状态。为保证除输入剥锚外没有第二个 prompt/repair 变量，score-blind 只允许在默认推荐任务段下创建，且仅采样首轮 attempt 已被业务接受的 champion；主调发生 repair 或没有 accepted target 时整批跳过、零实验调用/样本。符合条件时复用首轮 accepted target 固化的实际 config/provider/model/endpoint/temperature/预算与路由归因，不重新查询可能已变化的路由。其 `input_hash` 覆盖中央调用准备完成后、实际发送前的精确语义请求（含最终消息、schema/JSON mode 与预算），不是从业务 DTO 拼出的摘要。输入用固化 seed 做确定性打乱，并在调用前原样落 `seed/input_order_json/input_hash`，候选集合不得增减，禁止事后从当前数据重建。

统一影子总闸 `llm_challenger` 缺省关；recommendation 模块级 running 锁让 prompt challenger 与 score-blind 互斥，同批全部影子 challenger 合计至多一次额外 LLM 调用，并发/重试通过事务内实验+批次重复检查与调用前 running 占位收敛为单一事实。score-blind 的成功、失败、空 picks、越池输出均只写实验 run、`ep1` 输出事实与调用审计，绝不改业务 picks/action/confidence、候选池、推荐批次状态或 `l2` 标签。`so1` 选股配对评估按 `experiment_type` 独立聚合 score-blind，重算只读本地事实、零 LLM；管理端显示实验类型、`sb1`、锁定协议、seed、精确输入 hash/顺序、运行状态及配对结果，并显著标注“纯影子、不影响推荐”。

评价协议必须在创建 score-blind 实验时预注册并锁定：短线 `[5,10]` 日、长线 `[20,60]` 日窗口固定，`min_effective_batches`、`max_coverage_drop_pct`、`max_severe_loss_rate_pct`（严重亏损仍定义为 `net_return_pct<-5%`）与 `multiple_testing_method` 必填；采样总目标至少为每类最小有效批次数的 2 倍，`protocol_json/protocol_hash/protocol_locked_at` 首写后不可修改，观察结果后不得换门槛。当前 so1 报表只按锁定窗口展示有效批次、覆盖率下降与严重亏损率护栏；协议有效批次要求 score-blind 与 champion 的 matched-K 收益事实成熟，不额外要求 Quant 组成熟，三方配对指标仍维持共同成熟口径。报表如实显示预注册的多重检验方法，但尚不生成 p 值、也不执行多重检验校正，因此不得据此宣称统计显著或自动作出晋级结论。上线验证以 `docs/ROADMAP.md` §5 的 S3-6C 清单为准；通过前只可报告“待验收”，不得声明线上有效或启动 S4/S5。
