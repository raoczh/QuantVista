...
> 状态：方案基线（v1.11；**P0 全部九项已实施**——第五批 P0-6 prompt 不可变快照与 L0-L3 分层收官，v1.10 修复批收口独立审查确认的六类实现缺口，v1.11 加固上游 60s/504 长调用；此前 P0-1 全部、P0-7 stale/PIT 与机读拒答码、P0-2 统一运行元数据、P0-8 调用关联/审计完整性、P0-3 证据链 ev4、P0-4 semantic validator、P0-5 capability matrix、P0-9 模块化输出预算均已实施）<br>
> 编制日期：2026-07-19；v1.4 修订：2026-07-20；v1.5 修订：2026-07-21（Asia/Shanghai）；v1.5 审查修复：2026-07-21（repair 漏标、完整性门禁、日报/新闻 PIT、拒答码端到端、取消审计脱敏）；v1.6：2026-07-21（P0-2/P0-8 实施）；v1.7：2026-07-21（P0-3/P0-4 实施）；v1.8：2026-07-22（P0-5/P0-9 实施）；v1.9：2026-07-22（P0-6 实施，P0 收官）；v1.10：2026-07-22（P0 修复批——独立审查确认缺口收口）；v1.11：2026-07-22（全 LLM 后台化后的上游 504 加固）<br>
> v1.11 摘要（上游 60s/504 加固）：**①真正 SSE**——Chat/Responses 流式 client 无整体 timeout，统一携带 `Accept: text/event-stream`、`Cache-Control: no-cache`、`Accept-Encoding: identity`，Transport 禁止自动压缩；全部业务 `chatCompletion` 默认流式优先，只有明确 4xx 拒绝 stream 才回落整包。**②预算不得丢失**——Chat 的 `max_tokens` 被拒时携同值改用 `max_completion_tokens` 并缓存目标能力；Responses 始终携带 `max_output_tokens`；无等价预算字段就失败，禁止去参后无上限生成。**③单次调用瘦身**——11 个业务模块上限均不超过 2500 token，analysis/qa/news 降至 2500、compare 降至 1000，结构化模块最多 1 次 repair，坏输出回灌默认 600 字，并同步收紧各 prompt 输出长度。**④任务边界**——分析、问答、AI 对比、白话选股与既有推荐/日报一样在独立后台 Context 中运行；后台化解决入站请求取消，SSE+预算解决模型网关 504，两层不可互相替代。<br>
