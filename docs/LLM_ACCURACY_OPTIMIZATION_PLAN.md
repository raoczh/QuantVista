...
> 状态：方案基线（v1.7；P0 第三批已实施——P0-3 字段路径证据链（ev4）+ P0-4 跨模块 semantic validator；此前 P0-1 全部、P0-7 stale/PIT 与机读拒答码、P0-2 统一运行元数据、P0-8 调用关联/审计完整性均已实施）<br>
> 编制日期：2026-07-19；v1.4 修订：2026-07-20；v1.5 修订：2026-07-21（Asia/Shanghai）；v1.5 审查修复：2026-07-21（repair 漏标、完整性门禁、日报/新闻 PIT、拒答码端到端、取消审计脱敏）；v1.6：2026-07-21（P0-2/P0-8 实施）；v1.7：2026-07-21（P0-3/P0-4 实施）<br>
...
> 工作区边界：v1.5 起 P0-1/P0-7 的实现代码随本批一并入库；P0-8 正文保留原文是已确定的产品决策，v1.7 后仅 P0-5/P0-6/P0-9 仍为计划项。
...
`llm_accuracy_contract` 当前只控制 P0-1 的 ac1 注入、结构化温度/repair 温度和 Chat/Responses 完整性门禁。v1.7 起 `llm_evidence_refs`（P0-3：快照 unknowns 注入 + evidence_id/source 标注）与 `llm_semantic_validator`（P0-4：仅新增跨字段规则；各模块既有专属校验不受控）已实施，均缺省开、管理后台可关。P0-7 的 stale/PIT、交易日历、新闻窗口和拒答码属于安全地板，始终启用；关闭兼容开关不得让旧行情、未来新闻或未知交易日历重新进入业务成功路径。
