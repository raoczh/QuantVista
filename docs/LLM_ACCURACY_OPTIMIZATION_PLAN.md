...
> 状态：方案基线（v1.3；三轮调研及角色/提示词适配完成；仅文档交付）<br>
> 编制日期：2026-07-19（Asia/Shanghai）<br>
...
> 工作区边界：本次只新增/修订本计划文档，不实施业务代码；当前工作树中的其他未提交文件属于并行开发，本文不把它们认定为本计划已落地成果，也不对其做回退。
...
截至本文核验，统一 `ac1` 准确性契约、结构化低温钳制、repair 轮温度归零和“缺结束事件即拒收”的流式完整性门禁均**尚未实施**；`server/service/llm_contract.go` 及对应测试当前不存在。这里的“repair 轮温度归零”只指 repair 请求的有效温度固定为 0，不代表取消 repair；repair 次数仍由模块显式上限控制。它们只作为 P0 计划项出现，不得写成工作区 WIP、已验证或已发布能力。
...
- `_refs/UZI-Skill/SKILL.md`、`skills/deep-analysis/assets/{data-contracts,quality-checklist}.md`、`skills/deep-analysis/references/task2.5-qualitative-deep-dive.md`、`task3-investor-panel.md`、`task4-synthesis.md`，以及 `scripts/lib/{agent_analysis_validator,self_review}.py`；
...
| P0 | 模型能力靠隐式判断 | 不同 provider 对 JSON schema/function calling/response 格式支持不同 | 兼容性失败时可能静默退化为自由文本 |
...
| P0 | 流式完整性和输出预算不统一 | 正常 EOF 缺结束事件仍可能成功；部分模块直接使用用户 `MaxTokens`，repair 也沿用原温度/预算 | 截断 JSON 或半截自由文本可能被当成成功，provider 差异无法归因 |
| P0 | 审计正文未脱敏且缺调用关联 | 日志保存消息/响应正文，但缺 prompt/data/schema hash、attempt、repair、structured method 和 finish state | 难以复现或串联 repair，用户问题、持仓等敏感内容还可能进入审计正文 |
...
本文以提交 `061f3d4` 为可复核代码基线，并将当前工作区的并行未提交改动视为外部输入。当前没有本计划新增的业务代码；`server/service/llm_contract.go`、`llm_contract_test.go`、`llm_context.go` 和 `llm_context_test.go` 均不存在，`ai_client.go`/`ai_client_responses.go` 也没有本计划所称的 `ac1`、低温或流式 EOF 门禁改动。

提交基线已有的 stale/fail-closed、风险闸门和交易计划校验可以作为现状能力记录，但本计划提出的统一契约、字段级证据、能力矩阵、流式完整性和跨模块语义校验均仍是“待实施”。当前工作树中 Claude 的并行修改不在本计划状态统计内，也不因本文而被覆盖或回退。

### 2.5 实际 LLM 调用入口矩阵

下表按 2026-07-19 工作树中的真实调用入口逐项审计，覆盖所有主要 LLM 交互；统一契约尚未落地，不能替代每个模块自己的 schema 和语义校验。

| 文件/模块 | 调用形态与现有解析/校验 | 主要准确性缺口 | 对应计划 |
|---|---|---|---|
| `server/service/ai_client.go` + `ai_client_responses.go` | chat/responses；流式优先；JSON object fallback；HTTP 重试、usage、调用审计；Scanner 错误会失败，但正常 EOF 缺少结束事件仍可能被接受 | 缺声明式 `json_schema/function_calling/json_object/free_text` 能力表；未统一记录 effective method、repair、prompt/data hash；半截响应门禁未完成 | P0-1、P0-2、P0-5 |
...
| `server/service/qa.go` | 自由文本/流式；问题长度、会话隔离、固定首轮快照、消息数上限；回答数字核验；失败删除空会话 | 自由文本无 claim schema；文本证据只有数字近似；多轮历史缺分层/裁剪审计；stale 拒答语义和截断不能程序判定 | P0-2/3/6/7/9、P2-3 |
...
| `server/service/compare.go` | 自由文本短评；只注入有效行情；长度提示、数字 evidence 核验、配额 fail-closed | “谁更值得关注”无结构化 action/证据路径；时效/数据冲突和截断状态未结构化；无 repair | P0-2/3/4/7/9 |
| `server/service/dailyreport.go` | `dailyReview` JSON、1 次额外 repair、坏输出截断回灌、数字 evidence；异步/partial/旧报告回滚 | 市场 claims 与推荐批次缺 evidence ID；各分支 coverage/structured method 不统一；新闻窗口、stale 语义和 attempt 关联需集中校验 | P0-2/3/4/6/7/9、P1-2/4 |
...
| `server/service/newsai.go` | 批量 JSON；id 映射；sentiment/scope/policy 枚举归一、分数 clamp、方向矛盾归零、板块白名单；失败规则降级 | 没有 repair/完整 coverage 检查；未知/重复/漏项可静默过滤；实时 latest-5 注入缺窗口、来源优先级、类别和发布时间契约，source alignment 未记录 | P0-2/4/5/9、P1-4/6 |
...
| `a_share_flow` | `market`、`quote`、`capital_flow`、`lhb`、`sentiment`、`events`、`turnover` | 龙头/题材/封板/换手、龙虎榜射程、T+1 与解禁质押风险；短周期失效条件 | 非 A 股、非交易时段快照或无射程 -> `skip + not_applicable_reason` |
...
所有模块成功、repair、拒答和降级都写同一组元数据（可先作为 JSON 列，稳定后拆表）。以下是目标契约示例，当前尚未实施：
...
```json
{
  ...
  "prompt_version": "p16",
  "prompt_hash": "sha256:...",
  "prompt_snapshot": "immutable-or-redacted-reference",
  "strategy_version": "s1",
  ...
  "attempt_count": 1,
  "output_budget": 2048,
  "finish_state": "done|finish_reason|completed|eof_without_marker|error",
  "coverage": 1.0,
...
`prompt_snapshot` 必须脱敏：API key、持仓隐私和用户自由文本可保存加密引用或 hash，管理员审计才可查看原文。内容 hash 与版本同时保存，版本不能替代 hash。
...
| L3 自定义任务段 | 关注角度、语气、排序偏好、用户问题；保存版本、内容 hash 和脱敏快照 | 是，但不能改变 L0/L1 |
...
|---|---|---|---|
| 不可覆盖准确性契约、结构化低温、repair 温度归零 | UZI/TradingAgents | 计划在中央客户端出口注入 `ac1`，结构化温度上限 0.2，repair 请求温度固定为 0；额外次数由模块显式上限控制，P0-9 统一后默认最多 1 次，不得隐式增加 | 待实施（P0-1/P0-9） |
| 流式优先但拒收半截响应 | stock-scanner/本项目实践 | 计划要求 `[DONE]`、`finish_reason` 或 Responses 完成事件；缺失时失败且不落库，非流式仅对明确不支持时回退 | 待实施（P0-1） |
...
|---|---|---|---|---|
| P0-1 | `ac1` 中央契约、低温和流式完整性 | 新建 `llm_contract.go` 并接入 `ai_client*.go`；`llm.go` 只接入独立 capability probe | 所有业务模块真实调用带不可覆盖契约；结构化有效温度<=0.2；repair 温度=0；chat 需明确终止标记/允许的 stop 或 tool_calls，`length/content_filter` 拒收；Responses 仅 `completed` 成功，`incomplete`/EOF/failed 不落库；探针能力与业务调用分开审计 | 待实施 |
| P0-2 | 统一运行元数据 | `model`、`llm_call_log`、analysis/recommendation/daily 表 | manifest 字段齐全；旧记录可读；hash 计算稳定 | 待实施 |
| P0-3 | 字段路径证据链 | `evidence_refs`、snapshot builder、前端展示 | 关键结论至少 1 个 evidence ID；缺失字段进入 unknowns | 待实施 |
| P0-4 | 跨模块 semantic validator | 新建 `llm_semantic_validator.go`，调用方保留专属规则 | rating/action/position/target/stop/buy zone/RR/risk gate 全部一致；失败不落“成功” | 待实施 |
| P0-5 | capability matrix | 新建 `model_capabilities.go`、provider smoke test | 每 provider/model 声明 schema/function/json/free_text、temperature、token、reasoning/tool 参数能力；错误路由可观察 | 待实施 |
| P0-6 | prompt 不可变快照与不可覆盖边界 | `prompt.go`、PromptTemplate 表、manifest、analysis/qa/daily/recommendation/review | 自定义任务段保存 hash/快照和版本；准确性、安全、stale、注入防护和输出 schema 由系统追加且不可覆盖；占位符错误可诊断 | 待实施 |
| P0-7 | stale/PIT fail-closed 统一 | analysis、qa、daily、news、trade_plan | 过期/无历史快照不得生成精确当前计划；拒答原因机读 | 基线部分已有；统一契约待实施 |
| P0-8 | 审计脱敏与完整性 | `llm_call_log.go`、`model/llmlog.go`、管理端 | 请求/响应可追踪；用户问题、持仓和敏感字段脱敏或加密引用；日志写失败不阻断业务且有告警；保存 schema/prompt/data/attempt/repair/finish 元数据 | 基础正文审计已有；脱敏、关联字段和完整性待实施 |
| P0-9 | 模块化输出预算与截断语义 | analysis、qa、compare、newsai、screener_ai、recommendation、daily、recbear | 每个模块声明 `MaxTokens`/字符上限；`finish_reason`、截断、空响应和 repair 进入统一错误码；预算超限不得静默当成功 | 待实施 |
...
任何版本发布同时满足（每个模块、每种数据状态至少 100 个样本；少于该数量报告“未评估”，不得把小样本当达标；比例按模块/状态分层并给出 95% 置信区间）：
...
`llm_accuracy_contract`、`llm_semantic_validator`、`llm_evidence_refs`、`llm_capability_router`、`llm_independent_panel`、`llm_reflection_shadow`、`llm_challenger`。
...
- `server/service/llm_contract.go`（拟新增）：`ac1` 契约、低温和流式完整性策略；
...
- Superpowers：`skills/test-driven-development/SKILL.md`、`skills/verification-before-completion/SKILL.md`、`skills/writing-skills/testing-skills-with-subagents.md`；
