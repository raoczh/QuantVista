...
> **文档状态：LLM 契约权威说明，离线开发项已收口。** 当前剩余工作是 `ROADMAP.md` §5 的线上部署验收和 `P3_GATE_ASSESSMENT.md` 所述真实样本积累；旧版本摘要用于追溯，不应被当成新的待开发批次。
>
> **v3.0 当前状态（2026-08-04）：代码已实现、待线上部署验收。** 本次补齐初版遗漏的三条调用层语义：①模型路由只替换调用目标，`chatParams.MaxTokens` 是路由前已经计算完成的业务预算，路由目标配置不得再次取小；因此 length/max_tokens repair 的 1.5 倍扩容跨路由保持不变。②Chat/Responses、流式/非流式 usage 只要任一真实分量非零就全部保留，上游 `total=0` 时只补 total：prompt/input 父分量非零时采用父分量，否则才以 cached 兜底；completion/output 父分量非零时采用父分量，否则才以 reasoning 兜底；两侧相加得到 total。即使异常上游报告的子分量大于非零父分量，也不能改成取大或把子分量重复相加；只有全部 usage 缺失时才整包估算。③reasoning/cached 别名取大不累计；显式 reasoning 与位于首个非空白位置、完整闭合且可跨 chunk 的 `<think>...</think>` provider 块分列审计，不进入业务正文；未闭合或嵌入业务正文的字面标签原样保留，避免误删。v2.6 摘要中的“路由目标输出预算取严不放大”只记录当时实现，已被此条覆盖，不再是现行语义。可执行线上验收见 `docs/ROADMAP.md` §5“LLM v3.0 调用层语义收口批”；在该清单完成前不得声称已线上验证。<br>
> 状态：方案基线（v3.0；**LLM 调用层对标 new-api 收口批已实施——预算语义反转（用户 max_tokens 原样生效）+截断 repair 扩容+reasoning/缓存观测+截断归因，详见 v3.0 摘要**；v2.9 计划定夺批已完成——P0-5 剩余能力声明位（json_schema/function_calling/reasoning）定夺为不扩、§5.2 watch_items/next_check_at 定夺为不升格（均为文档定夺零代码行为变化，理由与重启前置条件见 §7.1 P0-5 行与 §5.2 定夺注）；至此计划内可离线开发项全部清零，剩余=部署验收人工清单与 P3（等真实样本，见 docs/P3_GATE_ASSESSMENT.md）**；v2.8 计划遗留收口批已实施——P1-6 golden 回归集满额（18 角色逐角色 ≥2 known-answer + ≥1 edge-case 机读登记+程序校验，来源黑名单/cold-data fixture 补齐）+ 清「confidence 为落库终值」校准口径限制（推荐条目落库保存模型原始口头置信度快照，校准报表与联合评估「原始 vs 终值」双口径分列）**；v2.7**P0 全部九项已实施，P1-1~P1-9 已实施，P2 全部六项（P2-1~P2-6）已实施——v2.7 收口外部审查对 P2 批次的 4 项修复（回滚对象一致性/联合评估换手锁定段隔离/路由后批次归因/发布审计绑定实验基线），v2.6 收官 P2-4 模型路由与成本优化 + P2-6 自动发布门（P1-9 遗留的「LLM 只复核缺口」审计员并入 promote 门 6），v2.4 同时收口两轮独立审查对 P1 批次的修复（反思跨用户隔离/claim 正文核验段/新闻窗口 nw2/辩论 db2 程序收口/校准孤儿剔除与 §8.1 门槛对齐）**——第五批 P0-6 prompt 不可变快照与 L0-L3 分层收官，v1.10 修复批收口独立审查确认的六类实现缺口，v1.11 加固上游 60s/504 长调用，v1.12 取消业务 prompt 输出篇幅限制，v2.0 实施 P1-1 推荐 coverage/越池诊断，v2.1 实施 P1-2 claim/evidence/invalidator + P1-4 新闻窗口来源对齐 + P1-6 固定回归集，v2.2 实施 P1-3 条件式独立 bull/bear/judge 辩论 + P1-5 反思记忆影子层，v2.3 实施 P1-7 校准与后验标签 + P1-8 角色/提示词资产 registry，v2.5 实施 P2-5 联合评估 + 校准分层 + P2-3 多层上下文；此前 P0-1 全部、P0-7 stale/PIT 与机读拒答码、P0-2 统一运行元数据、P0-8 调用关联/审计完整性、P0-3 证据链 ev4、P0-4 semantic validator、P0-5 capability matrix、P0-9 模块化输出预算均已实施）<br>
> 编制日期：2026-07-19；v1.4 修订：2026-07-20；v1.5 修订：2026-07-21（Asia/Shanghai）；v1.5 审查修复：2026-07-21（repair 漏标、完整性门禁、日报/新闻 PIT、拒答码端到端、取消审计脱敏）；v1.6：2026-07-21（P0-2/P0-8 实施）；v1.7：2026-07-21（P0-3/P0-4 实施）；v1.8：2026-07-22（P0-5/P0-9 实施）；v1.9：2026-07-22（P0-6 实施，P0 收官）；v1.10：2026-07-22（P0 修复批——独立审查确认缺口收口）；v1.11：2026-07-22（全 LLM 后台化后的上游 504 加固）；v1.12：2026-07-22（取消业务 prompt 输出篇幅限制）；v2.0：2026-07-23（P1-1 推荐 coverage/越池诊断实施）；v2.1：2026-07-23（P1-2/P1-4/P1-6 合并批实施）；v2.2：2026-07-23（P1-3/P1-5 合并批实施）；v2.3：2026-07-23（P1-7/P1-8 合并批实施，P1 收官）；v2.4：2026-07-23（P2-1 champion/challenger 实验 + P2-2 假设→实验→反馈实施；P1-9 硬检门并入 promote；两轮审查修复收口）；v2.5：2026-07-24（P2-5 组合/回测联合评估 + 校准报表分层 + P2-3 多层上下文检索实施）；v2.6：2026-07-24（P2-4 模型路由与成本优化 + P2-6 自动发布门实施，P2 收官）；v2.7：2026-07-26（外部审查 P2 修复批——4 项全属实全修）；v2.8：2026-07-26（计划遗留收口批——P1-6 golden 满额 + 原始置信度双口径）；v2.9：2026-07-27（计划定夺批——P0-5 剩余声明位不扩 + watch_items/next_check_at 不升格，纯文档定夺）；v3.0：2026-08-03（LLM 调用层对标 new-api 收口批——预算语义反转/截断扩容 repair/reasoning 观测与归因/去 200000 上限）<br>
> v3.0 摘要（LLM 调用层对标 new-api 收口批——根治推荐等模块 finish_reason=length 稳定截断）：**【预算语义反转】**`capModuleTokens` 由「与模块预算取小」反转为「用户配置正数原样生效」（仅保留 `llmGlobalHardCap`=MaxInt32/2 整型溢出护栏，对齐 new-api relay/helper/valid_request 同量级；此前用户在配置里调大 max_tokens 恒被压回模块上限，错误文案「可调大 max_tokens」是死路）；模块预算表降级为「用户未配置（0）时的默认正文预算」并全表重定标（recommendation/experiment 2500→6000 恒等（单变量对照纪律）、analysis 2500→5000、qa/position_advice→4000、rec_review/daily_report/news→3000、rec_bear/reflection/release_audit/screener_parse→2500、debate 三角色→2000、trade_plan→2000、analysis_review/compare→1500、debate_rebuttal→1200）——旧值系 2026-07-14「压进上游 60s 整包窗口」定标，流式 client 无整体超时后该前提已不成立；`llm_configs.max_tokens` 去 200000 业务上限（validate 仅 ≥1，前端 input 同步去 max），新建缺省 2048→8192（对齐 new-api Claude DefaultMaxTokens 缺省）。**【截断扩容 repair】**预算表增 `RepairTokenMultiplier`（缺省 1.5）：上一轮 finish_state=length/max_tokens 时下一轮预算 ×1.5（`moduleRepairTokenCap`，仅溢出保护），且**截断轮不回灌半截输出**（`appendModuleRepairMessages` 统一收口——半截 JSON 无修复价值只占上下文；结构校验失败仍按 RepairFeedChars 截断回灌），11 个带 repair 模块（analysis/analysis_review/recommendation/rec_review/rec_bear/daily_report/screener_parse/debate×4 经 debateCallOne/reflection/release_audit/position_advice）统一接线——修复此前「截断后按原预算原样重试必然二次截断」的确定性失败。**【reasoning/缓存观测与截断归因】**usage 解析增 `reasoning_tokens`/`cached_tokens`（chat `completion_tokens_details`/`prompt_tokens_details` + responses `output_tokens_details`/`input_tokens_details` + DeepSeek 系 `prompt_cache_hit_tokens` 等别名，同义字段取大不相加防重复记账）、`reasoning_content` 与响应正文分列落库（chat `reasoning_content`/`reasoning` 字段 + responses reasoning 输出项 + 正文内嵌 `<think>` 流式跨 chunk 剥离——思考文本不得进 extractJSONObject 与前端正文）；`llm_call_logs` 增三列（reasoning_tokens/cached_tokens/reasoning_content），AdminLlmCalls 列表统计条+详情把 finish_state=length 拆「思考吃光预算（reasoning_exhausted：reasoning×2>completion）/正文写满（content_exhausted）」两类归因。**【token 字段与缓存对齐】**o1/o3/o4/GPT-5 主动切 `max_completion_tokens`（new-api IsOpenAIReasoningOModel/IsGPT5Model 同款判定+运行时 4xx 回落兜底），该字段含隐藏 reasoning token，`requestTokenBudget` ×2 预留（优于 new-api 原值搬移——正文预算不被思考挤占）；recommendation/analysis 携带 `prompt_cache_key`（module:prompt_version，上游拒绝该参数时回落重试）。**【输出量定夺：p15 不限输出】**p14 曾按旧 2500 预算收紧推荐输出（三数组各 ≤3 条、rejected 只列前 3 且 ≤30 字），随预算语义反转与截断扩容 repair 落地，该瘦身同批被 p15 撤销（用户定夺：不为省 token 限制输出而影响用户分析与程序判断）——恢复「名单内全部未入选标的的落选理由」与不限条数，截断风险由预算与扩容 repair 兜底而非压缩输出；TestPromptOutputLengthLimitsRemoved 恢复对推荐 prompt 的篇幅限制禁扫。**【定夺：不引入用户级思考配置】**不在 LLM 配置暴露 reasoning_effort/thinking_budget（用户定夺）：请求侧不主动注入思考控制参数，保持网关/模型默认——new-api 中继语义下客户端不发即默认行为，思考控制走网关侧模型名约定（-thinking/-nothinking 后缀）；观测侧（reasoning_tokens/归因）不受影响。审计口径：ListLLMCalls 三查询（count/归因统计/分页）各自独立重建 GORM 链，不复用 finisher 后的链。<br>
> v2.9 摘要（计划定夺批，纯文档定夺零代码行为变化——清计划内最后两个「待后续/待定夺」表述）：**【P0-5 剩余能力声明位：不扩】**json_schema/function_calling/reasoning 参数声明位定夺为不引入。评估事实：全部 12 条业务链路的结构化输出统一为 `response_format:{type:json_object}`+服务端 JSON 解析与 schema 校验（structured_method 两态词表 json_object/free_text），无任何调用发送 json_schema/注册 tools/携带 reasoning 参数（finish_reason=tool_calls 仅作 P0-1 门禁的兼容终态接受，非主动能力）；P2-4 路由的能力消费点也只有 json_object 一维。能力矩阵的观察层建立在真实请求的 4xx 反馈上——没有请求形态，观察层永远学不到、声明化路由无 unsupported 可跳过，扩声明位=登记一个恒 unknown 且无消费方的假账（与 P1-8「§4.5.1b 方法角色不登记防假账」同纪律）；reasoning 系模型对外的真实兼容差异（拒 temperature、要求 max_completion_tokens）已被现有 temperature/max_tokens 两个观察维覆盖，无需独立 reasoning 维。重启前置条件：某业务模块真实改用 json_schema/function_calling 请求形态时，在 llmCapability 枚举与 `model/llmlog.go` 的 LLMStructured* 词表同步扩展。**【§5.2 watch_items/next_check_at：不升格】**原「留待 P1-3 消费时定夺」已到期（P1-3 v2.2 实施且辩论角色不消费这两字段），定夺为维持现状：可证伪目的已由 invalidators 全链路达成（claims/交易计划/长线 invalidation/辩论强制 invalidator）；watch 语义已有承载且受核验（分析 Suggestions 进「关注点」核验段、长线 ReviewCycle 声明复查节奏），升格=同语义双轨；next_check_at 要求模型输出绝对时间戳违反「可确定性计算在 Go」纪律（§6.3）——复查时点由程序从 valid_days/交易日历推导，即既有 tracking/ReviewNeeded 机制。重启前置条件：出现真实消费方（如提醒系统按结构化观察项自动建监控规则），届时按「程序推导时间、模型只提观察维度」分工设计。**至此计划内可离线开发项全部清零**：P0 九项/P1 九项/P2 六项全部实施，已知限制与待定项全部清账或固化为有意不做；剩余=ROADMAP §5 人工验收清单（用户执行）与 P3（立项门槛 0/6，等真实样本积累，快照见 docs/P3_GATE_ASSESSMENT.md §7.4 指针）。<br>
> v2.8 摘要（计划遗留收口批，非新增功能——清计划内声明过的已知限制）：**【P1-6 golden 满额】**对照 §8.1「每个启用角色至少 2 个 known-answer + 1 个 edge-case」硬门槛收口——registry（llm_roles.go）的 `CounterExamples` 单数组拆分为 `KnownAnswers`/`EdgeCases` 两数组（KA=固定输入→精确断言预期输出：手工验算/表驱动正例/端到端具体值；EC=边界与异常路径：失败降级/剥除/拒绝/flag 关/旧记录兼容），18 角色逐角色登记满额；`TestLLMRoleCounterExamplesExist`（存在性校验）升级为 **`TestLLMRoleGoldenQuota`（满额校验）**：每角色 ≥2 KA + ≥1 EC、坐标真实存在于 *_test.go、两数组不重复登记，缺额直接红（防「先登记后补测试」假账）。补 §8.1 声明的缺失 fixture 四个（llm_golden_test.go）：**来源黑名单**（TestGoldenNewsSourceBlacklistNoise——日报事件降噪黑名单 eventNoiseWords 即来源质量黑名单的实现形态：盘面播报类标题即使高优先级高分词也零注入+关键锚词防瘦身+全噪声输入空事件不硬凑）、**cold-data**（TestGoldenColdDataFixtures——长期无新信息标的的机读 unknowns 声明精确匹配（缺失≠为零、禁记忆补齐）+冷门股空新闻窗口 unavailable 仍声明边界）、**复核员解析 known-answer**（TestGoldenAnalysisReviewParse——verdict 大小写归一/confidence clamp/comment 截断假 LLM 端到端）、**辩论 bear 失职反例**（TestGoldenDebateBearEmptyChallengesRepair——challenges 引用全非法被剥空=失职触发 repair、二轮仍失职 bear_failed 连 bull 一起丢、主结果零改写）。PASS/FAIL audit fixture 已由 P2-6 批 TestParseReleaseAudit/TestLLMExperimentAuditGate 承担（登记进 release_audit 角色坐标）。**【原始置信度双口径】**清 P1-7/P2-5 Notes 声明的「confidence 为落库终值」口径限制——`recPick` 新增 `RawConfidence *int`（json:raw_confidence，随 DetailJSON 落库不加列；指针语义：nil=旧记录无快照，0 是值域内合法值不与「无快照」混淆）：`snapshotRawConfidence` 在 parse+normalize 之后、applyReviews 之前快照（与 rawActionBySym 同位——applyReviews 的 reject 压 25/复核覆盖是 Confidence 唯一改写点）；幂等不覆盖（repair 重入防御）；normalizePick 剥除模型自附（服务端快照字段不可伪造）；量化降级兜底 picks 不快照（程序赋值 35 非模型预测，恒 nil 如实）。消费侧纯测量零门控：校准报表（reccalib.go）与联合评估（recjointeval.go）共用 `calibRawSummary` 单一实现增 `raw_calib` 分列（sample=有快照 buy 样本/missing=旧记录如实单列不硬造/diverged=被复核真实改写数/Brier/ECE 与主口径同门槛 calibEvalMinSample）——主校准恒吃终值（用户实际看到的置信度），原始口径单独测「模型自身预测」的校准性，两口径同池分列；旧「口径限制（如实声明）」Notes 换「双口径」声明。**【小项】**AdminLlmCalls.vue moduleLabel 补 release_audit（第五十五批验收观察项 1）；experiment 影子 run 不进批次 LlmRunJSON **维持现状**（v2.4 限制⑨评估结论：影子 run 已完整落 llm_experiment_runs 结构化对照行（valid/picks/overlap/token/延迟/Error）且回指主调 trace/run，llm-calls 凭同 trace_id 可查逐请求审计——归并进批次 LlmRunJSON 只是第三处冗余展示位，且 fillBatchRunMeta 在影子采样后不再重刷（时序注定缺席），改时序引入回写风险换不来新信息，不做）。<br>
> v2.7 摘要（外部审查 P2 修复批，codex 4 项）：**①回滚对象一致性（高）**——`RollbackLLMExperiment` 修复前只校验状态 promoted 即无条件按 PrePromote 锚恢复：实验 A 晋级后模板被编辑或实验 B 再晋级（champion 指针前移），回滚 A 会覆盖更新的 champion/B 的晋级产物。修复：回滚前 `experimentRollbackStale` 校验当前启用模板 hash == 该实验 ChallengerHash，不符拒绝并指引提示词页 revision 快照；实验列表（LLMExperimentView.RollbackStale）透出原因，前端禁用按钮并展示。**②联合评估换手的锁定段隔离（高）**——修复前换手查询无条件吃全部日期标签（含 locked 日期），默认页面请求已消费并展示受锁定段影响的换手指标却无审计登记，违反「默认只显示 locked 范围与样本数」。修复：默认视图（!includeLocked）换手查询以 dev 段末日为界（仅在存在锁定段时加界），include_locked（已登记审计）才含全量日期；Notes 与前端 tag 声明口径。**③路由后批次归因记真实目标（中）**——修复前批次 Provider/Model 在调用前记原配置，路由只改调用参数与 llm_call_logs：校准分层 provider·model、联合评估对照与路由 Brier 回退（routeCalibBrierPair 按目标模型查）全部从批次归因取数，路由流量误归入原始模型层——目标模型无直接样本时 Brier 回退拿不到数据，有样本时用的也不是路由流量的表现。修复：`applyBatchRouteAttribution`（主调后、落库前）在 mainRun.routeApplied.Applied 时把批次 Provider/Model 改写为路由后真实目标；LLMConfigID 保持原配置指针（回显/重试语义），原→新归因看 manifest routed 与 llm_call_logs。**④发布审计绑定实验基线（中）**——修复前审计读「审计当下」的当前模板（默认 champion 只传占位说明），工件只绑 ChallengerHash：创建后或审计通过后 champion 被编辑，旧 PASS 仍可晋级，单变量复核缺稳定依据。修复：实验创建时固化 `ChampionContent` 内容锚（默认 champion=实际内置任务段，`promptModuleDefaultTaskSegs` 与提示词页默认展示单一权威）；`releaseAuditChampionContent` 恒吃实验锚；工件落 `ChampionHash`；promote 增门 5b（当前 champion 未偏离创建锚——偏离=对照失效须重建实验）+门 6b（工件 ChampionHash 与实验锚一致——旧版无绑定工件/基线变动后旧 PASS 须重审）。<br>
> v2.6 摘要（P2-4 模型路由与成本优化 + P2-6 自动发布门，P2 收官）：**【P2-4】模型路由（新建 `llm_router.go`+`model/llm_router.go` 的 `llm_module_routes` 表，flag `llm_model_routing` **缺省关**——行为开关非回滚开关，llm_challenger 同款 == "true" 语义）**——按模块把 LLM 调用改走管理员指定配置（一行=一个模块，目标配置须属启用管理员）：**路由只换调用目标**（BaseURL/APIKey/Model/EndpointType/Temperature；输出预算取严不放大），业务 prompt/schema/模块预算/repair 纪律/配额记账主体（发起用户）零改动；挂点=中央客户端 `chatCompletion`/`chatCompletionStream` 两公开出口（P0-5 applyCapabilityRouting 同款收口，且在契约/能力路由**之前**——温度钳制与能力路由作用于路由后最终目标），凭 chatMeta.Module 命中，探针（module=test）与 Module 空恒不路由。**按任务选择结构化能力（P0-5 矩阵消费）**：结构化任务（JSONMode）不路由到已知不支持 json_object 的目标（观察带 TTL，目标恢复后自动重新路由）；free_text 任务不受限。**自动回退（§10.2 阈值）持久化且不自动恢复**：健康检查在路由缓存重载（TTL 30s）时评估，三信号任一触发即写 auto_fallback_at/reason 停用该路由并 SysWarn，管理端显式恢复（reset/重新保存）才重新生效——①失败率（近 24h 该 module×配置的 llm_call_logs error 占比 ≥30%，样本 ≥5；「准确率下降」的调用层代理）；②成本比（路由目标平均 token / 同模块其他配置平均 token > MaxCostRatio，默认 1.35 对齐 §8.1；双方样本 ≥5 才可比）；③校准分层 Brier（仅 recommendation：**校准报表 provider·model 分层数据的路由消费点**——路由目标层 Brier 比最优已评估层恶化超 10% 即回退；只读缓存不在调用路径重算，两层都已评估才可比、样本不足不下结论）。**experiment 模块恒跟随 recommendation 路由**（不可单独配置——champion 与 challenger 必须打同一目标，否则 §9.3 单变量对照失效，测试锁定）。审计：llm_call_logs 逐请求记路由后真实 config/provider/model；业务 manifest 增 `routed` 声明（原配置→路由目标归因）。管理端：AdminSettings「模型路由」卡（路由表 CRUD/健康快照/自动回退原因/显式恢复）+「LLM 准确性契约」卡第九开关。**【P2-6】自动发布门（新建 `llm_release_gate.go`+`llm_release_audits` 工件表）**——P2-1 promote 五道程序硬检之上补齐三件：①**LLM 复核缺口（P1-9 遗留收官）**：发布审计员角色（module=release_audit，ra1/release_audit.v1，registry 第 18 角色，预算 1500/1）只复核程序硬检覆盖不了的内容性缺口（challenger 是否削弱系统契约与安全纪律/是否夹带 hypothesis 未声明的额外变更（单变量）/是否含诱导伪造数据措辞/影子指标与 improved 结论是否矛盾），**程序收口防口是心非**：verdict 封闭枚举 pass/fail、fail 必附 findings、high 级 finding 程序强制 fail；LLM 不可用如实报错、输出 repair 打满落 verdict=error 工件（error 不是 FAIL 判定但同样挡晋级）——**不出假 PASS**；工件逐次落库全量保留（走系统默认配置，管理员手动触发计次）。②**promote 门 6**：最新审计工件必须 PASS 且其 ChallengerHash 与实验当前内容一致（内容变过须重审）——「未 PASS 不进入 synthesis」（P1-9）在此收口；promote 仍是管理员显式动作（人工审批不被 LLM 替代）。③**一键切回 champion**：晋级瞬间固化 PrePromote 锚（当时启用中的模板状态），`RollbackLLMExperiment` 按锚恢复（晋级前有自定义模板=恢复其内容生成新 revision；晋级前默认模板=停用自定义模板，内容保留不删工件），状态 promoted→**rolled_back**（终态，不可 abandon）。实验页：发布审计按钮+工件列表（verdict tag/findings/trace/token）+一键切回按钮。<br>
> v2.5 摘要（P2-5 联合评估 + 校准分层 + P2-3 多层上下文）：**【P2-5】组合/回测联合评估（新建 `recjointeval.go`，`GET /api/admin/llm-joint-eval`，管理端「联合评估」页）**——纯测量零门控零 LLM 调用：把推荐标签事实升维到组合视角，收益（胜率/均值/中位/P10/严重亏损率）、Alpha、最大回撤（信号日串联净值回撤+标签级 MAE 并列）、换手（同用户同类型相邻批次 buy 名单新进占比与重合率）、成本拖累（gross−net 均值；执行模拟按次日开盘价成交无滑点项，如实声明不硬造滑点数字）、覆盖率（标签结算状态全分布）与校准（Brier/ECE，门槛与校准报表同源）同屏。**§9.1 时间切分落地**：唯一信号日升序前 70% 归开发段（dev，调参迭代随便看）、其余归锁定测试段（不足 10 个信号日不切分、锁定段如实缺席）；**锁定段纪律=默认只显示范围与样本数，`include_locked=1` 显式请求才计算指标且每次读取登记审计**（options 表 KV 持久化计数+时刻，管理页醒目展示「已读 N 次」——「发布前只读一次」以可见审计落地而非硬禁）；include_locked 恒重算不走缓存（缓存命中不得绕过审计）；**版本/来源对照（prompt_version/provider·model 分组）只吃开发段样本**——prompt_version 对照即 champion/challenger 晋级前后的收益对照落点（P2-2 声明的「晋级后 P2-5」闭环），锁定段隔离防「以对照之名反复消费锁定段」。样本口径与校准报表共用 `loadRecCalibSamples` 单一权威（l2/next_open/rec_id>0，matured 中 forced/orphan/degraded 单列剔除）。**【校准分层】**（reccalib.go 扩展，清 v2.4 声明的口径限制）：校准报表每 type 增 slices——策略/regime（标签归因冗余）与 provider·model/prompt_version（关联批次归因列，批次已删/旧行空值归「（未知）」）四维分层（buy 口径与口头置信度校准同样本池）；每层样本 ≥calibEvalMinSample 才产出该层 Brier/ECE（小样本层只报命中率/收益供参考），单维超 12 个取值按样本降序保留、其余合并「（其他）」行（合并行不产出校准指标）；分层是观察不是采信。「confidence 为落库终值（混合复核修正）」的限制仍如实保留声明。**【P2-3】多层上下文检索（新建 `qa_context.go` + recreflect.go 检索分层，flag `llm_layered_context` 缺省开）**——QA 侧（q12→q13）：被 qaHistoryLimit(12) 裁剪的更早轮次不再静默丢弃，纯程序化三层=Tier1 最近消息全文（消息流位置与截断行为不变）+Tier2 被裁轮次截断索引（每轮「Q 要点/A 要点」一行，1200 字符预算从近到远，预算尽即停防跳轮索引、更早轮计 dropped）+Tier3 按需检索（本轮问题与被裁轮次 rune bigram 相关性评分 ≥3 才命中、top 2 轮注入更完整摘录）；注入为 system 数据段（【历史会话分层上下文】，声明摘录非当前快照事实、旧回答结论可能过时）；**token 与被裁剪字段可见**：每轮回答落 `context_json`（QaContextLayers：各层条数/字符、完全不可见轮数、粗估 token、Tier3 命中依据），前端消息徽章+tooltip 展示「模型看到了什么/没看到什么」；核验语义零变化（历史 user 消息本就全量进值域、assistant 历史本就不进——分层注入不改变）；flag 关回退旧静默截断且消息序列与旧版逐字节一致（测试锁定），分层快照观测不受 flag 控。反思影子侧（影子快照 rf1→rf2，生成版本 rf1 不动、两版本常量自此拆分）：检索快照升级分层可观测——Tier1=同标的命中/Tier2=同策略命中计数、candidates_total 候选总数与 trimmed_count 超名额被裁数（「还有多少教训没进快照」可见）、Tier3=同策略全历史结局聚合统计（win/loss/止盈/止损计数+均收益，聚合概览无名额压力）；lookupReflections 签名与三不纪律零改动，分层统计恒带 userID+available_from 过滤（用户隔离与防回放泄漏铁律延续）；`llm_reflection_shadow` 控检索与否、`llm_layered_context` 控分层结构，两 flag 正交。<br>
> v2.3 摘要（P1-7 校准与后验标签 + P1-8 角色/提示词资产 registry）：**【P1-7】校准报表（新建 `reccalib.go`，`GET /api/admin/llm-calibration`，管理端「LLM 校准报表」页）**——纯测量零门控：把「程序合成置信度档位（SysConfidence）」与「模型口头置信度（0-100）」当预测信号对后验标签算校准。推荐侧吃 l2/next_open/rec_id>0 的 matured 标签（代表持有期短线 h=10/长线 h=20，与反思、月度走查同口径；Forced 强平与量化降级条目单列剔除——降级置信度 35 是程序赋值非模型预测），口头置信度限 buy 算 Brier/ECE/可靠性曲线（固定 5 桶，Gap=命中率−平均置信度），SysConfidence 分档命中率/中位净收益/**成本前 gross 与成本后 net 分开**/alpha/严重亏损率+单调性观察（只描述不判定）；动作 precision（P(net>0|buy)）/recall（P(buy|net>0)）与 watch 对照命中率并列，alpha 口径分列；coverage=标签结算状态全分布。分析侧无落库标签按 hindsight 同款口径现算（个股标准分析评级方向 vs 基准日+20 交易日收盘，neutral 不判、上限回看 400 条），SysConfidence 从 ResultJSON 解析（旧记录无值单列计数）。**§6.3 纪律**：报表是「口头 confidence 不当真实概率」的测量证据不是采信；**§8.1 门槛**：总样本 <30 evaluated=false 如实「未评估」（Brier/ECE 为 null 非 0 分），单桶/单档 <5 标注样本不足。全库口径不分用户（管理端测量样本才够门槛）。**【P1-8】角色资产 registry（新建 `llm_roles.go`，`GET /api/admin/llm-roles`，管理端「LLM 角色资产」页）**——**代码内声明表+测试锁定一致性**（llm_budget.go 同款模式；不建第二套 prompt 模板注册表——用户自定义仍走 PromptTemplate）：16 个真实接线角色（11 业务+4 辩论+反思）逐个登记版本锚（**引用代码版本常量编译期绑定**，不手抄）/schema_version/目标/适用市场 horizon/触发条件（适用性路由的声明——辩论三条件/反方只对 buy/复核可选等既有条件编排）/输入白名单/必答要求/禁止动作/失败降级/反例测试坐标；预算从 llmModuleBudgets 回填（单一权威不重复声明）。锁定测试五组：registry↔预算表键集双向一致（新增模块必须两表同登记）、必填字段齐全、schema 锚读源码对拍 newLLMRun 调用处、**反例坐标读 *_test.go 源码校验存在性**（挪测试须同步 registry）、§6.3 反投票合成三重锚（全局纪律+judge ForbiddenActions+debateJudgeSystem prompt 含「不按角色票数平均」）。纯资产登记零行为变化：registry 不参与运行时路由决策，只声明并锁定既有条件编排。**P1-9 处置**：明确延后至 P2-1——其「未 PASS 不进入 synthesis」预设资产晋级/发布流程（draft→champion），当前全部角色是生产路径固定编排无晋级动作可门控；质量门的代码硬检部分（字段/时效/来源/空值/截断）已作为 P0-1~P0-9+P1-2 运行时校验存在（每次调用都检查，强于发布抽检）。待 P2-1 champion/challenger 实验流程建立后作为其发布门实施。<br>
> v2.2 摘要（P1-3 条件式独立 bull/bear/judge + P1-5 反思记忆影子层）：**【P1-3】条件式辩论（新建 `analysis_debate.go`，版本 db1，flag `llm_conditional_debate` 缺省开）**——仅个股标准分析（非 panel/非 as_of/非历史解释）在信任层回填之后按程序判定触发（§6.2 边界落地）：低置信度（SysConfidence=low，含复核 reject 级联后）/证据冲突（claims 存在 contradictory）/风险闸门临界（**warn 级**——block 不触发：语义校验已硬约束 block ⇒ 不得 bullish，辩论无从改变结论）；高置信默认单路零额外成本。**编排=蓝图 C 落地**：bull 立论（引用 evidence_id+至少一条 invalidator）→ bear 反驳+立论（逐条 challenges 回应 BULL_CLAIMS、confirmed 区分已证实/假设风险）→ 条件式 rebuttal（程序判定：双方 claims 引用**同一 evidence_id** 的对立解读才给 bull 一次反驳，各说各话直接裁决）→ judge 裁决（verdict/decisive/rejected/unresolved_claim_ids/confidence_reason/invalidators/conflict_note；prompt 纪律=按证据质量排序不按票数平均、neutral 只能用于证据真正平衡）。**防伪造纪律（P0-3/P1-2 延续）**：触发原因程序判定进 manifest（bull run 携带 DebateManifestInfo：trigger_reasons/rounds/max_rounds=3/call_budget=4）；evidence_id 必须指向核验引擎产出的证据白名单（越界剥除）、claim id（bu-/be- 前缀）程序重编号、judge 的 *_claim_ids 校验剥除非法引用、block 时 verdict=bullish 程序拒绝触发 repair（防触发判定重构旁路的收口）、模型自附 result.debate 在 parseAnalysisResult 剥除。**辩论是附加复核不是裁决替换**：不改写主 rating/summary/claims；judge verdict 与主评级方向相反时压 SysConfidence=low 并点名（同 review reject 级联先例）；任一角色失败=整体降级为既有单路（bull/bear 失败丢弃半成品、judge 失败保留双方论点无裁决），记 debate.degraded_reason。调用预算：+3~4 次/触发（bull/bear/rebuttal 条件式/judge 各 repair ≤1，预算表 debate_bull·bear·judge 1200/rebuttal 800）。**【P1-5】反思记忆影子层（新建 `recreflect.go`，版本 rf1，flag `llm_reflection_shadow` 缺省开）**——两半独立：①**反思生成**挂 tracking job 标签结算之后：成熟标签（l2/next_open/真实推荐）总量 ≥30 才启用（S2-1 排序原则）、每轮 ≤5 条控成本、只反思代表持有期（短线 h=10/长线 h=20，walk-forward 月度走查同款口径）；LLM 挂系统默认配置（resolveNewsLLM 同款：后台任务 token 记配置所有者审计不扣次）、批量一次调用（固定三问 2-4 句教训）、idx 关联程序校验越界丢弃、outcome/factor_digest/available_from 全程序产出（LLM 只写教训不判输赢）；落 recommendation_reflections（唯一键 rec_id+horizon 幂等，NOT EXISTS 排除已反思行）。②**影子检索**挂推荐 runGeneration：按 LLM 名单标的（同 symbol 优先）+策略（同 strategy+recType 补足）检索 ≤5 条适用教训快照落批次新列 `reflection_json`——**三不纪律（regime/bear/quality 影子同款）：不注入 prompt（buildMessages 零改动）、不改写 action/置信度、拒选与降级批次也落**；注入转正必须凭注入前后影子配对评审。**防回放泄漏铁律**：检索恒过滤 `available_from <= 检索时点`（AvailableFrom=反思生成时刻）——线上实时恒真，但它是历史回放不把未来结算带回过去的唯一防线，测试锁定。前端：分析页「多空辩论复核」区（触发原因 tag/双方论点与交锋/裁决与 decisive·rejected 标记/降级说明）、推荐结果头「反思记忆（影子）」tag（tooltip 教训明细+影子声明）；管理后台「LLM 准确性契约」卡第五/六开关。**主链路行为边界**：主分析 schema（analysis.v1）与 prompt（p20）不变（debate 是服务端回填字段+独立调用）；推荐 picks/评级/拒答行为零变化；影子三不纪律、P0 九项、P1-1 诊断、P1-2 claim 推导纪律不可回退基线不动。<br>
> v2.1 摘要（P1-2 claim/evidence/invalidator + P1-4 新闻窗口来源对齐 + P1-6 固定回归集）：**【P1-2】结论级 claims（核验引擎 ev4→ev5，新建 `llm_claims.go`）**——analysis/recommendation/daily 三模块的关键结论升格为结构化可追踪对象（`evidenceCheck.claims[]`：claim_id/section/text/evidence_ids/status/invalidators）。**防伪造纪律延续 P0-3**：claim 与 evidence_id 的关联由 `deriveClaims` 从核验明细程序推导（该段 origin 空的快照命中项 ID），禁模型自报；status 三态程序判定——段内存在 direction_mismatch=contradictory（最强告警优先）、有快照佐证=resolved、其余=unresolved（含无数字段与全复述命中——复述不算佐证，与 key_section 同口径）。claim 段清单：分析=总结（失效条件取 kill_switches）+交易计划（失效条件取计划 invalidators）、推荐=每 pick 一条（text 长线取 thesis/短线取首条理由，失效条件取 invalidation）、日报=总结+明日计划（无失效条件字段如实为空不硬造）。**invalidator 模型输出侧**：`tradePlan` 新增 `invalidators[]`（计划失效条件——与止损价互补的形态/逻辑作废声明，attachTradePlan 收尾 `normalizeInvalidators` 归一：去空/截 120 字/限 4 条，模型未输出如实为空），schema **trade_plan.v1→v2**、prompt **p19→p20**；长线 pick 补 `invalidation` 输出要求（短线既有），schema **recommendation.v1→v2**、prompt **p12→p13**。前端 TrustBadges 明细新增「结论追踪」折叠区（三态色 tag+佐证 evidence_id 回指+失效条件），tooltip 对 contradictory 点名警示；交易计划卡展示失效条件、推荐卡失效条件放开长线。**【P1-4】新闻窗口与来源对齐（newsai.go，版本 nw1）**——个股快照 news 块两分支（有新闻/无新闻）均带机读 `window_meta`：window_start/window_end（既有 [now-7d,now] 硬窗口显式化——窗口外 0 注入由查询双边界保证并加测试锁定，未来记录恒不注入）、source_coverage（P1~P5 优先级档计数）、source_alignment（程序化方向对齐分类：aligned/mixed/divergent/single_source/unavailable，阈值版本化 nw1 主导比 0.7；单源不得写成共识、模型只解释分歧不得重新分类——蓝图 F 纪律进 note）；`newsBrief` 补 priority/scope 来源元数据；空窗口 alignment=unavailable 且仍声明窗口边界（「窗口内确无」与「没查」区分）；as_of 回溯快照的 news 缺口沿用既有机读 unknowns 声明（P0-3），不重复建窗口元数据；日报事件窗口由 P0-7 上界纪律承担，本批不重复做。**【P1-6】固定回归集（新建 `llm_golden_test.go`+`llm_claims_test.go`）**——golden 文件承担「回归集目录」：P0 各批已锁场景（无 key/截断/流中断/repair 打满/越池诊断/冲突输出/stale fail-closed/prompt 快照/预算登记）以坐标索引指路不重复实现；新增锁定：P1-2 claim 三态 known-answer×2+edge（全复述不 resolved/旧记录兼容/文本截断）、P1-4 对齐分类表驱动 8 例+窗口声明 known-answer+窗口外 0 注入 DB 端到端（8 天前旧闻+未来记录+别家标的均不注入）、恶意自定义模板全模块枚举（promptModuleContracts 全键×「只输出纯文本」注入→契约分界头后置压制+输出协议锚点恒在，qa 自由文本模块锚点=「不下达买卖指令」）、注入文本数据段隔离（标题指令不影响程序化窗口声明、序列化不产生结构逃逸）、分析信任层端到端（fillAnalysisTrust 后 claims/invalidators/plan-origin 不算佐证）。**主链路行为边界**：claims/window_meta 均为纯观测新增（快照进 prompt 改变 data_hash 属预期）；picks/评级/拒答行为零变化；影子门控三不纪律、P0 九项、P1-1 诊断不可回退基线不动。<br>
> v2.0 摘要（P1-1 推荐 coverage/越池诊断）：**①程序化计数与机读错误码**——`parseAndFilterPicks` 剥除不再静默：每个输出条目恰归一类（covered/truncated/duplicate/out_of_pool/unknown，不变式 input=各计数之和），`RecCoverageDiag` 携带 input/unique/covered 计数、三类样本数组（每类 ≤10 个、单个截 16 字符，完整数量看 count 字段）、机读错误码（`rec_pick_out_of_pool`/`rec_pick_unknown_symbol`/`rec_pick_duplicate`/`rec_pick_overflow`）与 coverage=(covered+truncated)/unique（无输出的合法空拒选=1）；「越池」（6 位数字形态但池外，疑训练记忆选股）与「无法识别」（名称/乱码/空白）分类；rejected 的越池/重复/空剥除计 `rejected_dropped`（「已入选不算落选」的正常剥除不计）；全越池触发 repair 的错误文案带分类计数。**②manifest coverage 填充真实值**——`LLMRunManifest.Coverage` 从 P0-2 起的预留恒空开始填充：仅推荐主调 run 挂载（callWithRepair 取最后一轮有结构输出的诊断——repair 打满仍失败的降级批次靠它归因，坏 JSON 轮 diag=nil 不覆盖上一轮），其他模块恒省略不得伪造；`prompt_trimmed` 由 runGeneration 层填充（池内合格 kept 只有量化排名前 maxLLMCandidates 只进 LLM 的输入侧裁剪声明）。**③量化 fallback 保留 screen score**（语义核对确认既有实现已满足并加测试锁定）——fallback picks 按量化排名（Rank）升序、Reason/Evidence 以文本+`score=/rank=` 机读双形态携带量化分，DegradedSource 经 normalizePick 不剥；结构化 QuantScore/QuantRank 由 runGeneration 信任层回填（fallback 替换发生在信任层之前同一循环生效）。**④前端透出**——推荐结果头仅在有异常时显示「AI 输出剥除」警示 tag（tooltip 带样本与说明），正常批次零噪声；管理端凭批次 trace_id 在 /admin/llm-calls 关联逐请求审计（P0-2 已就绪）。**诊断是观测不是门控**：picks/rejected 行为与版本号（p12/s9/recommendation.v1）不变，越池仍剥除、超量仍截断、全越池仍 repair、宁缺毋滥拒选语义不动。<br>
> v1.12 摘要（取消 prompt 输出篇幅限制）：分析、交易计划、分析复核、推荐、推荐复核、推荐反方、日报、问答、AI 对比、新闻增强和白话选股的生产 prompt 均删除字数、句数及仅为压缩生成耗时设置的数组条数限制。真正 SSE、后台任务、模块 `max_tokens`、repair 次数与回灌裁剪保持不变；JSON schema、必填字段、推荐数量、固定角色、新闻逐 ID 映射、选股树复杂度和交易计划价格关系等业务契约继续保留。解析后的数据库/UI 安全截断也不属于 prompt 限制，仍保留。<br>
> v1.11 摘要（上游 60s/504 加固）：**①真正 SSE**——Chat/Responses 流式 client 无整体 timeout，统一携带 `Accept: text/event-stream`、`Cache-Control: no-cache`、`Accept-Encoding: identity`，Transport 禁止自动压缩；全部业务 `chatCompletion` 默认流式优先，只有明确 4xx 拒绝 stream 才回落整包。**②预算不得丢失**——Chat 的 `max_tokens` 被拒时携同值改用 `max_completion_tokens` 并缓存目标能力；Responses 始终携带 `max_output_tokens`；无等价预算字段就失败，禁止去参后无上限生成。**③单次调用边界**——11 个业务模块上限均不超过 2500 token，analysis/qa/news 降至 2500、compare 降至 1000，结构化模块最多 1 次 repair，坏输出回灌默认 600 字。**④任务边界**——分析、问答、AI 对比、白话选股与既有推荐/日报一样在独立后台 Context 中运行；后台化解决入站请求取消，SSE+预算解决模型网关 504，两层不可互相替代。<br>
> v1.10 摘要（P0 修复批，六项）：**①prompt 不可变运行快照**——`loadPromptRuntime` 一次读取同一模板行固化 `promptRuntime` 快照（原始正文/hash/revision），正文渲染（Render）与版本归因（Version）消费同一份；analysis/analysis_review/recommendation/daily_report/qa 五类消费点全部统一（此前 `promptOverrideFor` 与 `promptVersionFor` 各自查库，两次查询间模板被编辑会造成实际正文与记录版本不一致）；推荐把快照固化进 `recGenPlan`（同步段读取、后台段消费，异步窗口内模板编辑不再产生错位）。**②legacy 模板基线迁移**——`model.MigratePromptTemplateBaselines`（Migrate 内 AutoMigrate 后执行，事务化幂等）：升级前旧行回填 content_hash/合法 revision 并补建当前正文的基线快照，重复启动零写入、不覆盖已有历史、部分迁移状态（行已回填快照缺失等）可修复——升级后的首次修改/删除仍能回查升级前原文。**③capability 观察隔离与识别收紧**——观察 key 全维度（configID+规范化 provider+规范化 BaseURL+model+endpoint，配置换上游后旧观察天然失效）；`looksLikeUnsupportedJSONMode` 收紧为必须明确指向 response_format/text.format/json_object/json mode（「model/stream/temperature not supported」等泛 4xx 不再误判）；能力观察只在「去参 fallback 请求确认成功」后提交（审计 markJSONModeDropped 仍在重试前置位，两者职责分离），fallback 失败不污染能力状态。**④audit outcome**——完整性门禁拒收（Chat length/content_filter、Responses incomplete/failed、流式截断终态）时结果对象携带上游真实正文/usage/原始终态随错误返回：`writeLLMCallLog` 错误状态下 ResponseBody 保留真实正文（ErrorMsg 独立保存错误）、usage 与 finish_state_raw 照实落，`run.record`/manifest 拿到真实 raw state，各模块 token 统计计入拒收调用的实际消耗；业务调用方仍收到失败，半截正文不进成功解析/业务落库/API 响应。**⑤temperature/max_tokens 能力贯通**——静态声明+运行时观察 → `capabilitiesFor` 合并 → `applyCapabilityRouting` 消费（unsupported 直接省略参数，不再无条件发送）→ 运行时 4xx 识别（`looksLikeUnsupportedTemperature`/`looksLikeUnsupportedMaxTokens`，值超限不算参数不支持）→ 去参 fallback 确认成功后提交观察；max_tokens 因兼容性省略时预算失效是**可观察的降级**（观察存储+SysLog+审计 payload 三处可见），非静默无上限。**⑥最终有效请求单一事实来源**——`llm_call_logs.RequestBody` 落最终实际发送的完整 payload（含最终有效 temperature、实际 token 上限、结构化参数与 stream 形态；API key 走 Authorization 头天然不进 body）；manifest 的 structured_method 记录**最终实际生效**形态（run 内任一 attempt 回落即 free_text，多 attempt 汇总语义见 §5.1）；news run 接 `sentiPromptVersion`（n1）、compare 内联 prompt 新设显式版本 `comparePromptVersion`（c1）。§6.1 coverage 诊断行归位 P1-1（修正与 §7.2 的优先级矛盾）。<br>
> v1.9 摘要：**P0-6**——①内容 hash 与不可变快照：`PromptTemplate` 增 `content_hash`（sha256 hex 前 16，对渲染前原始内容计算）与 `revision`（内容变化才递增，只切 enabled 不动）；新表 `prompt_template_revisions` 每次内容变化落一行不可变快照（(template_id,revision) 唯一，递增基准取快照表 MAX 防失步撞索引；删除模板不级联删快照，归因链不断）；版本归因从裸 `-custom` 升级为 **`-custom.<hash8>`**（`promptVersionFor` 统一产出，旧行空 hash 读取侧现算）——同名版本必对应同一模板内容，manifest/llm_call_logs 的 prompt_version 可凭 hash8 回查当时模板原文；业务表 PromptVersion 列宽 16→32（llm_call_logs 24→32）。②L0-L3 分层：四个整段替换模块（recommend/daily/qa/review）的系统提示拆为「L3 任务段常量 + L1 契约常量」，默认整段=两常量**编译期拼接、与拆分前逐字节一致**（默认用户零行为变化，版本号不递增）；自定义模板降级为 L3 任务段注入——`composeCustomTaskPrompt` 在用户段之后强制追加【系统契约】分界头+模块契约（纪律/schema/输出纪律不可覆盖），Prompts.vue 展示每模块契约内容并提示旧「整段替换」模板已降级、建议删除自带 schema 段；分析 5 模块结构本就分层（Intro/OutputSpec 恒在）零改动。分析主记录版本归因收窄：只归因本模块 guidance 模板，复核员模板归因移至 manifest 里 review run 自己的 prompt_version（`analysisReviewSystemFor`），两模板各自可精确复现。③占位符可诊断：`lintPromptContent` 保存时返回警告（未知占位符/大小写错误/单花括号漏写/分层模块疑似自带输出格式），运行时宽容渲染不变；前端保存后逐条展示。L0（ac1 出口注入）/L2（程序构造快照）既有事实不变；L4 输出要求并入 L1 契约段。角色资产 ID（`bull_researcher@r1` 类）与模型侧 claims schema 属 P1-2/P1-8，本批不做。<br>
...
> 工作区边界：v1.5 起 P0-1/P0-7 的实现代码随本批一并入库；P0-8 正文保留原文是已确定的产品决策，v1.9 起 P0 九项全部实施完毕（均待部署验收），P1 起为计划项。
...
| 失败条件/失效开关 | 分析、推荐、交易计划统一 `invalidators[]/watch_items[]/next_check_at`（实施形态：invalidators 已全链路落地；watch_items/next_check_at v2.9 定夺不升格——见 §5.2 定夺注） | 把“看多”变成可证伪假设 |
...
`prompt_snapshot` 保存不可变引用或完整内容 hash；**管理端 LLM 调用审计不脱敏**——`llm_call_logs` 的请求/响应正文原样保留供管理员排障（仅长度截断防库膨胀，API key 走 Authorization 头本就不进 body）。内容 hash 与版本同时保存，版本不能替代 hash。v1.10 起 `llm_call_logs.RequestBody` 保存**最终实际发送的完整 payload**（含最终有效 temperature、实际 token 上限、结构化参数与 stream 形态——fallback 重试后覆盖为最终形态），中央客户端是最终 endpoint/structured method/有效温度/实际 token 参数/fallback 结果的单一事实来源；错误状态（完整性拒收）下 ResponseBody 保留上游真实正文、ErrorMsg 独立保存错误、usage 与 finish_state_raw 照实落（audit outcome）。
...
**多 attempt/repair 的 manifest 汇总语义（v1.10 确定性定义）**：manifest 是 run 级聚合——`finish_state`/`finish_state_raw` 取**最后一次 attempt** 的终态（`run.record` 每次调用后覆盖）；`structured_method` 记录整个 run **最终实际生效**的形态：run 内任一 attempt 发生 JSON mode 回落（声明化路由或运行时 fallback）即记 `free_text`——同一 run 的主调与 repair 轮同配置同目标，回落具备粘性（能力观察写入后由声明化路由接管），逐请求真值仍以 `llm_call_logs.structured_method` 为准；`attempt_count=1+repair_count` 不变式不变；`prompt_hash` 恒为初始消息形态（repair 轮不重算）。temperature/max_tokens 的最终有效值不进 manifest（run 级无单值语义——各 attempt 可因 fallback 不同），以 `llm_call_logs.RequestBody` 的逐请求最终 payload 为准。
...
> **v2.1 实施状态**：上述 claim 契约的核心子集已随 P1-2 落地（`llm_claims.go`，`evidenceCheck.claims[]`，ev5）——claim_id/section/text/evidence_ids/status/invalidators 齐全；**evidence_ids 与 status 由程序从核验明细推导（非模型自报，防伪造）**，invalidators 收集自模型输出的失效条件字段（tradePlan.invalidators / pick.invalidation / kill_switches）。
>
> **v2.9 定夺：`watch_items`/`next_check_at` 不升格为结构化 claim 字段（维持现状）**。原表述「留待 P1-3 辩论角色消费 claim 时一并定夺」——P1-3 已实施（v2.2）且辩论角色只消费 claims 的 text/evidence_ids/invalidators/status，不需要观察项与复查时点字段，定夺条件已到期，结论如下：①「把看多变成可证伪假设」的目的已由 invalidators 全链路达成（claims 失效条件+交易计划 invalidators+长线 invalidation+辩论各方强制 invalidator），watch_items 相对 invalidators 只是弱化表述（「关注」而非「作废」），升格不增加可证伪性；②watch 语义已有承载且已受核验——分析 `Suggestions` 进「关注点」核验段（analysis.go addSec，受证据核验与行动建议剥除纪律约束）、长线 pick 的 `ReviewCycle` 声明复查节奏，结构化升格=对同一语义建第二套字段（双轨），与「不提前造双轨」的原判断一致且 P1-3 后更无新消费方；③`next_check_at` 要求模型输出绝对时间戳，违反「可确定性计算必须在 Go 中完成」纪律（§6.3）——复查时点应由程序从 valid_days/review_cycle/交易日历推导，而这正是既有 tracking 追踪与 ReviewNeeded 机制已做的事。重启前置条件：出现真实消费方（如提醒系统要按结构化观察项自动建监控规则），届时按「程序推导时间、模型只提观察维度」的分工设计，不让模型写时间戳。
...
|---|---|---|---|
| 不可覆盖准确性契约、结构化低温、repair 温度归零 | UZI/TradingAgents | 中央客户端出口注入 `ac1`，结构化温度上限 0.2，repair 请求温度固定为 0；额外次数由模块显式上限控制（代码循环 attempt>0 置 `chatParams.Repair`，含 analysis/recommendation/dailyreport/screener_ai/recbear 与 analysis_review/rec_review；外部 manifest 的 `attempt` 采用 1 基），P0-9 统一后结构化模块最多 1 次，不得隐式增加（v1.11 已取消 analysis/trade_plan 的 2 次例外） | **已实施（P0-1，v1.5；repair 次数声明化随 v1.8 P0-9 落地，v1.11 收紧）** |
...
| 候选池内 LLM 精选与 coverage 诊断 | AlphaSift | 记录 unknown/duplicate/out-of-pool/prompt_trimmed 和 fallback | **已实施（v2.0 P1-1，与 §7.2 一致）**：程序化计数+机读错误码+manifest coverage 填充+fallback screen score 测试锁定；此前 v1.10 修正误标 P0 的内部矛盾 |
| 数据快照与 `as_of` | UZI/StockNova/FinGPT | 在已有快照/时效能力上统一 source/freshness/window 状态和 hash | 基线已有；统一契约待实施 |
| prompt registry 与不可变 hash | StockNova/RD-Agent | 自定义任务段保存快照/hash；准确性、安全和输出 schema 契约不可覆盖；champion/challenger 不直接覆盖 | **已实施（P0-6，v1.9）**：content_hash/revision + prompt_template_revisions 快照 + `-custom.<hash8>` 版本归因 + L1 契约系统追加；champion/challenger 实验属 P2-1 |
| recorder/metrics/artifacts | Qlib | 新增 manifest、评估工件、成本与风险指标 | P1 |
| 分层记忆与 reflection ID | FinMem/AlphaEvo | 先影子检索，按 `available_from` 过滤 | **已实施（v2.2 P1-5，与 §7.2 一致）**：反思生成+影子检索快照，`available_from` 防回放泄漏测试锁定 |

### 6.2 延后或有条件采用

- 独立 bull/bear/judge：只在低置信度、证据冲突或风险闸门触发时启用；高置信度默认单路，避免成本线性增长。**已实施（v2.2 P1-3）**：触发条件程序判定（low_confidence/contradictory_claims/risk_gate_borderline=warn 级；block 不触发——语义校验已硬约束，辩论无从改变结论），高置信默认单路零额外成本落地。
...
| P0-5 | capability matrix | 新建 `model_capabilities.go`、provider smoke test | 每 provider/model 声明 schema/function/json/free_text、temperature、token、reasoning/tool 参数能力；错误路由可观察 | **已实施（v1.8；v1.10 收口观察纪律；v1.11 修复 token 预算回落）**：能力三态由内置声明与 12h 运行时观察合并，观察 key 覆盖 configID/provider/BaseURL/model/endpoint，换上游不继承旧观察；`applyCapabilityRouting` 在两个公开出口消费 unsupported：json_object 改 free_text，temperature 可省略，Chat 的 `max_tokens` 则**携同值改用 `max_completion_tokens`**，Responses 的 `max_output_tokens` 永不省略。4xx 识别须字段明确指向且带不接受措辞，值超限不算；Chat/Responses 的流式与整包回落都采用“请求形态确有进展”的有限循环，兼容网关逐项报错时也不会在 token 切换前失败。能力观察只在最终 fallback 成功后提交，失败不污染 12h 缓存；两种 Chat token 字段都被拒或 Responses 拒绝预算字段时 fail closed，禁止无上限请求。provider smoke 仍独立审计且不注业务 prompt，supported 不代表业务推理可用；flag 只控制声明化路由，不关闭隐式兼容与观察。P2-4 多模型路由已实施（v2.6，消费本矩阵 json_object 维）。**json_schema/function_calling/reasoning 声明位定夺为不扩（v2.9）**：全部 12 条业务链路的结构化统一为 `response_format:{type:json_object}`+服务端 schema 校验，无任何调用发送 json_schema/tools/reasoning 参数——能力观察建立在真实请求的 4xx 反馈上，没有请求形态则观察层永远学不到、路由层无 unsupported 可跳过，扩声明位=为登记而登记的假账（与 §7.2 P1-8「方法角色不登记防假账」同纪律）；reasoning 系模型的真实兼容差异（拒 temperature、要求 max_completion_tokens）已被现有 temperature/max_tokens 两个观察维覆盖。扩维前置条件：某业务模块真实改用 json_schema/function_calling 请求形态，届时在 llmCapability 枚举与 LLMStructured* 词表同步扩展（`model/llmlog.go` 枚举注释已指向本定夺） |
| P0-6 | prompt 不可变快照与不可覆盖边界 | `prompt.go`、PromptTemplate 表、manifest、analysis/qa/daily/recommendation/review | 自定义任务段保存 hash/快照和版本；准确性、安全、stale、注入防护和输出 schema 由系统追加且不可覆盖；占位符错误可诊断。落地约束（v1.4）：扩展既有 PromptTemplate/PromptService，不建第二套平行注册表；`model/prompt.go` 的 `module` 列现为 `size:16`，角色资产 ID（如 `bull_researcher@r1`）入库前先评估列宽或建独立资产表；现状 recommend/daily/qa/review 四个扩展模块的自定义是整段替换 system prompt（分析五模块仅替换中段），收紧为 L0-L3 分层会改变存量用户模板的行为，必须提供迁移策略（存量模板降级为 L3 任务段注入并在界面提示），不得静默改变输出 | **已实施（v1.9，待部署验收）**：①`PromptTemplate` 增 content_hash（sha256 前 16，对渲染前原始内容）/revision（内容变化才递增），新表 `prompt_template_revisions` 落不可变内容快照（(template_id,revision) 唯一、递增基准取快照表 MAX、删除模板不级联删）；版本归因统一 `promptVersionFor` 产出 `base-custom.<hash8>`（旧行空 hash 读取侧现算），业务表 PromptVersion 列 16→32、llm_call_logs 24→32——manifest 与审计的 prompt_version 可归因到具体模板内容。②四个整段替换模块拆「L3 任务段+L1 契约」两常量，默认整段=编译期拼接与拆分前逐字节一致（默认路径零变化、版本号不递增）；自定义走 `composeCustomTaskPrompt`（用户段+【系统契约】分界头+契约段），纪律/schema 恒不可覆盖；Prompts.vue 展示各模块契约与迁移提示（旧整段模板已降级为任务段注入，建议移除自带 schema）；`Modules()` 的 default 改返回任务段防「以默认为模板」拷入契约造成重复。分析主记录版本只归因 module 模板，review 模板归因在 manifest 的 review run（`analysisReviewSystemFor`）。③`lintPromptContent` 保存时诊断（未知占位符/大小写/单花括号/分层模块疑似自带输出格式），响应 warnings 前端逐条展示；运行时宽容渲染保持。④v1.10 修复批：**运行快照统一**——`loadPromptRuntime` 一次读取模板行固化 `promptRuntime`（正文/hash/revision），Render 与 Version 同源，五类消费点（analysis/analysis_review/recommendation/daily_report/qa）统一消费快照、推荐固化进 recGenPlan（后台不重新查库）；**legacy 基线迁移**——`model.MigratePromptTemplateBaselines`（AutoMigrate 后自动执行，事务化幂等）为升级前旧行回填 content_hash/revision 并补建基线快照，升级后的首次修改/删除仍能回查升级前原文（此前依赖「首次再保存时补建」，首次修改即永久丢失旧正文）。角色资产 ID/模型侧 claims schema 属 P1-2/P1-8 未做 |
...
| P0-8 | 审计完整性（**正文保持原文**） | `llm_call_log.go`、`model/llmlog.go`、管理端 | 请求/响应可追踪且**原文可读**（管理员排障）；日志写失败不阻断业务且有告警；保存调用关联与 schema/prompt/data/attempt/repair/finish 元数据 | **已实施（v1.6；v1.10 修复批补 audit outcome 与最终 payload）**：调用关联/完整性字段已随 P0-2 落地（trace/run/parent/attempt/repair/structured_method/schema/prompt/data hash/finish_state+raw）；管理端列表按 trace_id/run_id 筛选、详情展示元数据与 hash；正文原样保留（60KB 截断）不变，严禁重新引入正文脱敏。v1.10 起：**RequestBody=最终实际发送的完整 payload**（含最终有效 temperature/token 上限/结构化参数，fallback 重试覆盖为最终形态；探针路径回退消息序列）；**完整性拒收也保留真实审计（audit outcome）**——Chat length/content_filter、Responses incomplete/failed 与流式截断终态在解析出正文/usage/原始终态后拒收时，ResponseBody 保留上游真实正文、ErrorMsg 独立保存错误、usage 与 finish_state_raw 照实落，run/manifest 拿到真实 raw state（此前拒收即丢弃，审计只剩错误文案、finish_state 靠文案还原）；业务调用方仍收到失败，半截正文不进成功路径 |
| P0-9 | 模块化输出预算与截断语义 | analysis、qa、compare、newsai、screener_ai、recommendation、daily、recbear | 每个模块声明默认 `MaxTokens`、repair 次数/回灌上限/截断扩容倍率；`finish_reason`、截断、空响应和 repair 进入统一错误码；预算超限不得静默当成功 | **已实施（v1.8；v3.0 已覆盖 v1.11 的“与模块上限取小/全部≤2500”历史口径）**：`llm_budget.go` 是所有业务 LLM 模块默认正文预算、RepairAttempts、RepairFeedChars 与 RepairTokenMultiplier 的唯一权威。用户 `max_tokens` 为 0 时取模块默认；用户明确配置正数时原样生效，只保留整型溢出保护，模块默认与路由目标配置均不得二次压低。当前默认值：recommendation/experiment=6000、analysis=5000、qa/position_advice=4000、rec_review/daily_report/news=3000、rec_bear/reflection/release_audit/screener_parse=2500、trade_plan/debate bull·bear·judge=2000、analysis_review/compare=1500、debate_rebuttal=1200。结构化模块至多 1 次额外 repair，自由文本模块为 0；上一轮 length/max_tokens 时下一轮预算按 1.5 倍扩容且不回灌半截输出，普通结构校验失败才按字符上限回灌。生产 prompt 不设字数、句数或性能型数组条数上限；截断、空响应与流式缺终态继续 fail closed，repair 打满归 `llm_output_invalid` 或模块既有确定性降级。逐请求最终预算以 `llm_call_logs.RequestBody` 为事实来源，部署后同时观察首块/总耗时、截断率与 repair 成本。 |

### 7.2 P1：独立复核、来源质量与评估集

| 编号 | 工作项 | 主要落点 | 交付物/门槛 |
|---|---|---|---|
| P1-1 | 推荐 coverage/越池诊断 | `recommendation.go`、`recfactor.go` | 候选池外、未知、重复代码均有错误码；fallback 保留 screen score。**已实施（v2.0，待部署验收）**：`parseAndFilterPicks` 程序化归类计数（`RecCoverageDiag`：input/unique/covered/truncated/unknown/duplicate/out_of_pool + 样本数组 + `rec_pick_*` 机读错误码 + coverage），剥除不再静默但行为不变（观测非门控）；manifest coverage 从预留恒空开始填充（仅推荐主调 run，其他模块不得伪造）；prompt_trimmed 声明输入侧 Top-N 裁剪；量化 fallback 的 screen score 语义（Rank 排序+score=/rank= 机读证据+信任层 QuantScore 回填）测试锁定；前端结果头异常警示 tag |
| P1-2 | Claim/evidence/invalidator | analysis、recommendation、daily schema | claim 可追踪、可标记 resolved/contradictory；每个计划有失效条件。**已实施（v2.1，待部署验收）**：核验引擎 ev4→**ev5**（`evidenceCheck.claims[]`，`llm_claims.go` 的 `deriveClaims`）——claim 与 evidence_id 关联程序推导（该结论段 origin 空的快照命中项，禁模型自报，防伪造纪律延续 P0-3）；status 三态程序判定（direction_mismatch=contradictory 优先 > 有佐证=resolved > 其余=unresolved，全复述命中不算佐证）；invalidator 模型输出侧：tradePlan 新增 `invalidators[]`（schema trade_plan.v2、prompt p20，normalizeInvalidators 归一、未输出如实为空）+长线 pick 补 `invalidation`（短线既有；schema recommendation.v2、prompt p13）；claim 段清单：分析=总结(kill_switches)+交易计划(invalidators)、推荐=每 pick(text 长线 thesis/短线首条理由)、日报=总结+明日计划(无失效字段不硬造)。前端 TrustBadges「结论追踪」区+交易计划失效条件卡。§5.2 的 watch_items/next_check_at **定夺为不升格（v2.9，维持现状）**：P1-3 已实施且辩论角色不消费这两字段，可证伪性已由 invalidators 全链路达成，watch 语义由 Suggestions（受「关注点」核验段约束）/ReviewCycle 承载，next_check_at 时间戳属程序推导域（tracking/ReviewNeeded 既有机制）——完整理由与重启前置条件见 §5.2 v2.9 定夺注 |
| P1-3 | 条件式独立 bull/bear/judge 与方法角色 | `analysis_panel.go` 或独立 service、prompt assets registry | 仅低置信度/冲突触发；最多 2 观点+1 judge、辩论最多 3 轮；每方逐条引用证据和失效条件；失败降级为单路并记录。**已实施（v2.2，待部署验收；落点为独立 `analysis_debate.go`，方法角色 registry 部分属 P1-8 未做）**：触发=程序判定三条件（SysConfidence=low / P1-2 claims 含 contradictory / risk_gate warn 级；block 排除+judge 侧 block⇒非 bullish 双收口）；编排 bull→bear（challenges 逐条回应 bull claim id、confirmed 区分已证实/假设风险）→条件式 rebuttal（仅双方引用同一 evidence_id 对立解读时）→judge（verdict/decisive/rejected/unresolved_claim_ids/confidence_reason/invalidators/conflict_note，按证据质量不按票数）；claim id 程序重编号（bu-/be-）、evidence_id 白名单剥除、judge 引用校验、parseAnalysisResult 剥模型自附 debate；判决不改写主 rating（方向相反压 SysConfidence=low 点名，同 review reject 级联）；任一角色失败降级单路记 degraded_reason（bull/bear 失败丢半成品、judge 失败保留双方论点）；manifest 声明 trigger_reasons/rounds/max_rounds=3/call_budget=4（bull run 携带）；版本 db1、flag `llm_conditional_debate` 缺省开、预算表登记（bull/bear/judge 1200、rebuttal 800、repair ≤1）；前端分析页「多空辩论复核」区 |
| P1-4 | 新闻窗口和来源对齐 | `newsai.go`、snapshot | `window_start/end/source_coverage/source_alignment`；窗口外新闻为 0 注入。**已实施（v2.1，待部署验收）**：个股快照 news 块两分支恒带机读 `window_meta`（窗口边界=[now-7d,now] 显式化、source_coverage=P1~P5 档计数、source_alignment=程序化五态分类 aligned/mixed/divergent/single_source/unavailable，阈值版本 **nw1** 主导比 0.7）；`newsBrief` 补 priority/scope；窗口外（旧闻/未来记录）0 注入测试锁定；单源不得写共识、模型只解释分歧不重分类（纪律进 note 数据段，prompt 版本不递增）；as_of 快照缺口沿用 P0-3 机读 unknowns（不用当前新闻倒灌既有保障）；日报事件窗口由 P0-7 上界纪律承担不重复建 |
| P1-5 | 反思记忆影子层 | `recommendation_reflections`、retriever | 返回 reflection ID/适用条件；`available_from` 过滤；影子不改变线上动作。**已实施（v2.2，待部署验收；落点 `recreflect.go`）**：①反思生成挂 tracking job（标签结算后）：成熟标签 ≥30 门槛、每轮 ≤5、代表持有期（短线 h=10/长线 h=20）、批量单调用 idx 关联越界丢弃、outcome/factor_digest/available_from 程序产出（LLM 只写教训）、resolveNewsLLM 系统默认配置只审计不扣次、唯一键幂等；②影子检索挂推荐 runGeneration：symbol 优先 strategy 补足 ≤5 条，快照落批次 `reflection_json`（含 reflection ID/适用条件/available_from/matched_by），**三不纪律：不注入 prompt、不改写 action/置信度、拒选与降级批次也落**；`available_from <= 检索时点`防回放泄漏铁律测试锁定（TestReflectionAvailableFromFilter）；版本 rf1、flag `llm_reflection_shadow` 缺省开；前端推荐结果头影子 tag+tooltip 教训明细 |
| P1-6 | 固定回归集 | `server/service/*_test.go`、fixtures、prompt assets | 无 key/无数据/窗口外/冲突/截断/越池/repair、角色不适用、字段越界、prompt injection、custom 覆盖和角色分歧等 golden cases；每个角色至少 2 个 known-answer + 1 个 edge-case，含来源黑名单、cold-data 和 `PASS/FAIL` audit fixture。**已实施（v2.1 目录+核心 fixture；v2.8 满额收口，待部署验收）**：`llm_golden_test.go` 承担「回归集目录」——P0 各批已锁场景（无 key/截断/流中断/repair 打满/越池诊断/冲突输出/stale fail-closed/prompt 快照/预算登记/审计保真）以坐标索引指路（挪动既有测试须同步目录），不重复实现；v2.1 落地：P1-2 claim 三态 known-answer/edge（`llm_claims_test.go` 8 组）、P1-4 对齐分类表驱动+窗口外 0 注入 DB 端到端、恶意 custom 模板全模块枚举（契约后置压制+输出协议锚点）、注入文本数据段隔离、分析/推荐/日报信任层端到端、旧记录 schema 兼容。**v2.8 满额收口（§8.1 硬门槛达成）**：18 角色 known-answer/edge-case 坐标机读登记在 registry（llm_roles.go 的 KnownAnswers/EdgeCases 两数组取代 CounterExamples），`TestLLMRoleGoldenQuota` 程序校验「每角色 ≥2 KA + ≥1 EC、坐标真实存在于 *_test.go、两数组不重复登记」缺额即红；补齐声明缺失的 fixture：来源黑名单（TestGoldenNewsSourceBlacklistNoise——日报事件降噪词表 eventNoiseWords 即来源质量黑名单实现形态：黑名单标题零注入+锚词防瘦身+全噪声空事件不硬凑）、cold-data（TestGoldenColdDataFixtures——长期无新信息标的的机读 unknowns 精确匹配+冷门股空新闻窗口 unavailable 仍声明边界）、复核员解析 known-answer（TestGoldenAnalysisReviewParse）、辩论 bear 失职反例（TestGoldenDebateBearEmptyChallengesRepair）；PASS/FAIL audit fixture 由 P2-6 批 TestParseReleaseAudit/TestLLMExperimentAuditGate 承担并登记进 release_audit 角色坐标 |
| P1-7 | 校准与后验标签 | tracking/backtest/walkforward | Brier、ECE、precision/recall、coverage、alpha、成本后收益分开统计。**已实施（v2.3，待部署验收；落点为独立 `reccalib.go`——tracking 的标签事实（reclabel）是数据源，报表不改 tracking/backtest/walkforward 行为）**：推荐侧吃 l2/next_open/rec_id>0 matured 标签（代表持有期短线 h=10/长线 h=20；Forced 与量化降级条目单列剔除——降级置信度是程序赋值非模型预测）；口头置信度（0-100）限 buy 算 Brier/ECE/可靠性曲线（固定 5 桶+Gap），SysConfidence 分档命中率/中位与均值净收益/**gross 与 net 分开**/alpha/严重亏损率+单调性观察（只描述）；动作 precision/recall（net 与 alpha 两口径）与 watch 对照并列；coverage=标签状态全分布。分析侧 hindsight 同款口径现算（评级方向 vs +20 交易日，neutral 不判，回看 ≤400 条），SysConfidence 从 ResultJSON 解析。§8.1 门槛：样本 <30 evaluated=false「未评估」（Brier/ECE=null 非 0 分；v2.4 修正为 calibEvalMinSample=100）；§6.3 纪律：报表是口头 confidence 校准性的测量证据非采信。`GET /api/admin/llm-calibration`（refresh=1 重算，进程内缓存+互斥）+管理端「LLM 校准报表」页。**v2.8 双口径收口（清「confidence 为落库终值」限制）**：推荐条目落库时 `snapshotRawConfidence` 在复核改写前快照模型原始口头置信度（recPick.RawConfidence 指针随 DetailJSON 落库，nil=旧记录、0 是合法值域；normalizePick 剥模型自附、量化降级恒 nil、幂等不覆盖）；校准报表与联合评估共用 `calibRawSummary` 增 `raw_calib` 分列（sample/missing/diverged + 与主口径同门槛的 Brier/ECE）——主校准恒吃终值（用户实际看到的），原始口径单独测模型自身预测的校准性，旧记录无快照如实计 missing 不硬造 |
| P1-8 | 角色/提示词资产 registry 与适用性路由 | `prompt_assets`、`model_capabilities`、analysis/recommendation/qa/news/review | 每个角色有版本、白名单、适用市场/horizon、必答问题、禁止动作、schema、预算和反例；同一快照下可回放；不以名人投票合成最终信号。**已实施（v2.3，待部署验收；落点为 `llm_roles.go` 代码内声明表——不建 prompt_assets 数据库表：角色是代码固定编排非用户数据，llm_budget.go 同款「声明表+测试锁定」模式）**：16 个真实接线角色（analysis/trade_plan/analysis_review/recommendation/rec_review/rec_bear/daily_report/qa/compare/news/screener_parse/debate_bull·bear·rebuttal·judge/reflection）逐个登记版本锚（引用代码版本常量编译期绑定）/schema_version/目标/适用市场 horizon/触发条件（既有条件式编排的机读声明）/输入白名单/必答要求/禁止动作/失败降级/反例测试坐标；预算由 llmModuleBudgets 回填（单一权威）。锁定测试：registry↔预算表键集双向一致、必填字段齐全、schema 锚读源码对拍 newLLMRun 接线、回归集坐标读 *_test.go 校验（v2.8 起 KnownAnswers/EdgeCases 满额校验 TestLLMRoleGoldenQuota——每角色 ≥2 KA + ≥1 EC）、§6.3 反投票合成三重锚（全局纪律+judge 禁令+debateJudgeSystem prompt 关键句）。可回放锚=trace_id+prompt_hash+data_hash（P0-2）+prompt_template_revisions（P0-6），registry 全局纪律声明。`GET /api/admin/llm-roles`+管理端「LLM 角色资产」页。§4.5.1b 方法角色（value_quality@r1 等）是未启用设计储备，不登记（防「登记了但代码不存在」假账） |
| P1-9 | 角色质量门与发布审计 | `quality_gate`、review artifacts、CI | 代码硬检先检查字段/时效/来源/空值/截断、`additionalProperties:false`、数组/字符串上限和 UNTRUSTED 隔离，再由 LLM 只复核缺口；输出 PASS/FAIL、grade、issues、missing_items、confidence_reason、backfill_tasks；未 PASS 不进入 synthesis。**明确延后至 P2-1 后实施（v2.3 决策）**：「未 PASS 不进入 synthesis」预设资产晋级/发布流程（draft→champion），当前全部角色是生产路径固定编排、无晋级动作可门控；其代码硬检部分（字段/时效/来源/空值/截断/服务端字段剥除）已作为 P0-1~P0-9+P1-2 运行时校验存在——每次调用都检查，强于发布时抽检。待 P2-1 champion/challenger 实验流程建立后，P1-9 作为其发布门实施。**v2.4 更新：代码硬检发布门已随 P2-1 实施**（PromoteLLMExperiment 五道硬检：状态机/无增量不晋级/样本量 ≥10/结构化有效率 ≥90% 从样本重算/challenger 内容 hash 防篡改；测试 TestLLMExperimentPromoteGate 逐门锁定）；「LLM 只复核缺口」的审计员角色与 PASS/FAIL 工件留 P2-6 自动发布门 |
...
| 编号 | 工作项 | 主要落点 | 交付物/门槛 |
|---|---|---|---|
| P2-1 | champion/challenger prompt 实验 | 新增 `llm_experiments`/artifact | 固定快照、协议、随机性、模型和 hash；候选只在影子流量运行。**已实施（v2.4，待部署验收；落点 `llm_experiment.go`+`model/llm_experiment.go` 两表）**：challenger 内容创建时拷贝固化（ChallengerHash）+champion 锚（当时启用模板版本串）；champion 指针=既有 PromptTemplate.enabled（晋级经 PromptService.Upsert 生成不可变 revision 快照并切指针，不删旧工件；回滚=提示词页恢复上一 revision）；影子采样挂推荐 runGeneration 主调成功后——同一批次候选名单只换 challenger 任务段一次调用（module=experiment、无 repair；v3.0 当前默认预算 6000，与 recommendation 恒等；run 回指主调 trace/run），输出只落 llm_experiment_runs（结构化有效性/picks 交集/coverage/token/延迟对照），**业务 picks 零改写（测试锁定）**；单变量纪律=同模块全局仅一个 running；采样只命中实验创建者本人（单变量对照+不烧他人 token）；flag `llm_challenger` **缺省关**；首版仅 recommendation 模块（llmExperimentSupportedModules 登记制）；管理端「LLM 实验」页（列表/新建/状态机动作/影子样本明细） |
| P2-2 | hypothesis -> experiment -> feedback | `research_runs`/DAG 或 JSON artifact | 记录预期改善、实际改善、失败原因、父版本；无增量不晋级。**已实施（v2.4，与 P2-1 同体落地——不另建 research_runs 表，实验行即研究工件）**：Hypothesis/ExpectedImprovement 创建必填（没有假设不立项）、ActualJSON=complete 时程序聚合（样本数/结构化有效率/picks 均值与重合度/token/延迟均值/失败样本原因；**决策质量收益对照不在影子阶段自动聚合——challenger 未真实落单无标签，属晋级后 P2-5**）、Conclusion（improved/no_improvement/worse）+非 improved 必填 FailureReason（失败原因是飞轮资产）、ParentID 串版本谱系；**无增量不晋级=promote 门 2 硬检**。P1-9 代码硬检发布门并入 promote（门 1 状态机/门 2 结论/门 3 样本 ≥10/门 4 有效率 ≥90% 从样本重算/门 5 内容 hash 防篡改），LLM 复核缺口部分留 P2-6 |
| P2-3 | 多层上下文检索 | qa/reflection | Tier1 当前摘要、Tier2 经验、Tier3 按需历史；token 和被裁剪字段可见。**已实施（v2.5，待部署验收；落点 `qa_context.go`+recreflect.go 检索分层，flag `llm_layered_context` 缺省开）**：QA 侧（q13）被 qaHistoryLimit 裁剪的更早轮次纯程序化三层注入——Tier1=最近消息全文（行为不变）、Tier2=被裁轮次截断索引（1200 字符预算从近到远、预算尽即停、更早轮计 dropped）、Tier3=按本轮问题 rune bigram 相关性检索（≥3 分命中 top2 轮注入摘录）；system 数据段声明「摘录非当前快照事实」；每轮回答落 context_json（各层条数/字符/完全不可见轮数/粗估 token/命中依据），前端徽章可核查；核验值域语义零变化；flag 关回退旧静默截断且消息序列逐字节一致（测试锁定）。反思影子侧检索快照 rf1→rf2（生成版本 rf1 不动）：Tier1/Tier2 命中计数+candidates_total/trimmed_count 被裁可见+Tier3 同策略全历史结局聚合统计；lookupReflections 与三不纪律零改动，分层统计恒带 userID+available_from 过滤。**有意不做 LLM 生成摘要**（Tier2 用截断索引换确定性与零成本——摘要质量诉求留积累后评估） |
| P2-4 | 模型路由与成本优化 | capability/router | 按任务选择模型和结构化能力；准确率下降或成本超阈值自动回退。**已实施（v2.6，待部署验收；落点 `llm_router.go`+`model/llm_router.go` 的 llm_module_routes 表，flag `llm_model_routing` 缺省关）**：按模块路由到管理员指定配置（目标须属启用管理员），挂中央客户端两公开出口（在契约/能力路由之前——后续钳制作用于最终目标）；只换调用目标，业务 prompt/schema/预算/repair/配额记账零改动；结构化任务不路由到 P0-5 矩阵已知不支持 json_object 的目标（观察 TTL 到期自动恢复）；自动回退三信号（失败率 ≥30%·近 24h 样本 ≥5 / 成本比 > MaxCostRatio 默认 1.35 / recommendation 的校准分层 provider·model Brier 比最优层恶化超 10%——P2-5 分层数据的路由消费点）持久化停用、管理端显式恢复防抖动；experiment 恒跟随 recommendation 路由（单变量纪律）；审计逐请求记路由后真实目标+manifest routed 归因；管理端「模型路由」卡 CRUD/健康快照 |
| P2-5 | 组合/回测联合评估 | backtest/tracking | 收益、Alpha、最大回撤、换手、滑点、覆盖率和校准同屏；锁定测试集不复用调参。**已实施（v2.5，待部署验收；落点为独立 `recjointeval.go` 只读报表——不改 backtest/tracking 行为，标签事实是数据源）**：`GET /api/admin/llm-joint-eval`+管理端「联合评估」页；每 type×代表持有期同屏收益（胜率/均值/中位/P10/严重亏损率）/Alpha/最大回撤（信号日串联净值回撤+标签级 MAE 并列，口径声明「等权逐批换仓近似非逐日盯市」）/换手（同用户同类型相邻批次 buy 名单新进占比+重合率）/成本拖累（gross−net；执行模拟无滑点项如实声明）/覆盖率/校准（Brier/ECE 与校准报表同门槛）；样本口径共用 loadRecCalibSamples 单一权威。**锁定测试集**：§9.1 时间切分（唯一信号日前 70% dev/后 30% locked，<10 天不切）；锁定段默认只出范围与样本数、include_locked=1 显式请求才计算且每次读取登记 options 表审计（管理页展示已读次数，读取恒重算不走缓存）；**版本对照（prompt_version/provider·model）只吃 dev 段**——prompt_version 分组即 P2-2 声明的「晋级后收益对照」落点。校准报表同批增设策略/regime/provider·model/prompt_version 四维分层（清 v2.4 口径限制声明） |
| P2-6 | 自动发布门 | CI/CD、管理端 | 通过阈值、人工审批、可一键切回 champion；保留完整运行工件。**已实施（v2.6，待部署验收；落点 `llm_release_gate.go`+llm_release_audits 工件表）**：通过阈值=P2-1 promote 门 1~5 程序硬检（既有）+**门 6 LLM 发布审计工件 PASS 且内容 hash 匹配**（P1-9「未 PASS 不进入 synthesis」收官）；审计员角色（release_audit/ra1，registry 第 18 角色）只复核程序硬检覆盖不了的缺口（契约削弱/单变量夹带/诱导伪造措辞/结论与指标矛盾），verdict 封闭枚举+fail 必附 findings+high 级 finding 程序强制 fail（口是心非收口）；LLM 不可用报错、输出无效落 verdict=error 工件——不出假 PASS；人工审批=promote 仍是管理员显式动作；一键切回=晋级瞬间固化 PrePromote 锚，RollbackLLMExperiment 对称恢复（promoted→rolled_back 终态），工件与 revision 快照全量保留不删 |

### 7.4 P3：规模化治理与长期储备

P3 不进入本轮代码交付；只有 P0-P2 在真实样本上达标后才立项。

> **2026-07-26 立项评估快照**（`docs/P3_GATE_ASSESSMENT.md`，纯测量）：P3 六项 **0/6 达门槛**——离线 golden 全绿（91 文件/587 测试），但线上影子与生产闸门指标全部「未评估」（可测环境真实样本=0：无成熟标签、无调用审计、无实验周期）；先决动作=执行 ROADMAP §5 部署验收开始积累样本，复测口径见该文档 §4。
...
`llm_accuracy_contract`、`llm_semantic_validator`、`llm_evidence_refs`、`llm_capability_routing`（原规划名 llm_capability_router，落地为此）、`llm_independent_panel`、`llm_reflection_shadow`、`llm_challenger`、`llm_layered_context`（v2.5 起，P2-3 多层上下文：缺省开——纯程序化零额外调用的回滚开关；关闭回退 QA 静默截断与 rf1 影子快照形态，上下文快照观测不受控）、`llm_model_routing`（v2.6 起，P2-4 模型路由：**缺省关**——改变业务调用目标属行为开关非回滚开关，llm_challenger 同款 == "true" 语义；路由表 per-module Enabled 与自动回退状态是第二/三重闸）。

`llm_accuracy_contract` 当前只控制 P0-1 的 ac1 注入、结构化温度/repair 温度和 Chat/Responses 完整性门禁。v1.7 起 `llm_evidence_refs`（P0-3：快照 unknowns 注入 + evidence_id/source 标注）与 `llm_semantic_validator`（P0-4：仅新增跨字段规则；各模块既有专属校验不受控）已实施；v1.8 起 `llm_capability_routing`（P0-5：仅控制声明化路由，能力观察与隐式回落不受控）已实施。四开关均缺省开、管理后台可关。P0-9 的模块预算/repair 次数声明不设 flag，是防止无界生成与连续长调用的确定性护栏；v1.11 收紧后的预算和最多 1 次 repair 不随兼容开关回退。P0-7 的 stale/PIT、交易日历、新闻窗口和拒答码属于安全地板，始终启用；关闭兼容开关不得让旧行情、未来新闻或未知交易日历重新进入业务成功路径。
