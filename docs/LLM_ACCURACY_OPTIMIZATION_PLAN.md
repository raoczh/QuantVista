...
`llm_accuracy_contract`、`llm_semantic_validator`、`llm_evidence_refs`、`llm_capability_routing`（原规划名 llm_capability_router，落地为此）、`llm_independent_panel`、`llm_reflection_shadow`、`llm_challenger`、`llm_layered_context`（v2.5 起，P2-3 多层上下文：缺省开——纯程序化零额外调用的回滚开关；关闭回退 QA 静默截断与 rf1 影子快照形态，上下文快照观测不受控）。
