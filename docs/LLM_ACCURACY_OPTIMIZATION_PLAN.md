...
| P0-9 | 模块化输出预算与截断语义 | analysis、qa、compare、newsai、screener_ai、recommendation、daily、recbear | 每个模块声明 `MaxTokens`/repair 回灌上限；`finish_reason`、截断、空响应和 repair 进入统一错误码；预算超限不得静默当成功 | **已实施（v1.8；v1.11 按上游 60s 窗口重标定；v1.12 取消 prompt 篇幅限制）**：`llm_budget.go` 是 11 个业务模块 MaxTokens/RepairAttempts/RepairFeedChars 的唯一权威，实际上限取用户配置与模块值的较小者。当前 analysis/recommendation/qa/news=2500，trade_plan/rec_review/rec_bear/daily_report=1500，analysis_review/compare=1000，screener_parse=2000；全部业务预算不超过 2500。结构化模块最多 1 次额外 repair，自由文本模块为 0；坏输出默认回灌 600 字、日报 800 字。生产 prompt 不再设置字数、句数或性能型数组条数上限，结构化与业务完整性要求不变。finish_reason=length/max_tokens、空响应与流式缺终态继续 fail closed；repair 打满归 `llm_output_invalid` 或模块既有确定性降级，manifest 记录 output_budget。预算过小会增加 length 拒收，过大则可能撞 60s/504，部署后须同时按首块/总耗时与截断率校准。 |
