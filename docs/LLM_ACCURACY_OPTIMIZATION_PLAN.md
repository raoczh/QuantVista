...
> 状态：方案基线（v1.10；**P0 全部九项已实施**——第五批 P0-6 prompt 不可变快照与 L0-L3 分层收官，v1.10 修复批收口独立审查确认的六类实现缺口；此前 P0-1 全部、P0-7 stale/PIT 与机读拒答码、P0-2 统一运行元数据、P0-8 调用关联/审计完整性、P0-3 证据链 ev4、P0-4 semantic validator、P0-5 capability matrix、P0-9 模块化输出预算均已实施）<br>
> 编制日期：2026-07-19；v1.4 修订：2026-07-20；v1.5 修订：2026-07-21（Asia/Shanghai）；v1.5 审查修复：2026-07-21（repair 漏标、完整性门禁、日报/新闻 PIT、拒答码端到端、取消审计脱敏）；v1.6：2026-07-21（P0-2/P0-8 实施）；v1.7：2026-07-21（P0-3/P0-4 实施）；v1.8：2026-07-22（P0-5/P0-9 实施）；v1.9：2026-07-22（P0-6 实施，P0 收官）；v1.10：2026-07-22（P0 修复批——独立审查确认缺口收口）<br>
...
| P0-5 | capability matrix | 新建 `model_capabilities.go`、provider smoke test | 每 provider/model 声明 schema/function/json/free_text、temperature、token、reasoning/tool 参数能力；错误路由可观察 | **已实施（v1.8；v1.10 修复批贯通 temperature/max_tokens 并收口观察纪律）**：`model_capabilities.go` 已建——能力三态（supported/unsupported/unknown）由「内置 provider 声明（openai 全 supported；任意兼容中转缺省 json_object=unknown——静态表无法穷举真实网关能力，运行时观察为主）+ 进程内观察存储」合并，观察优先于声明；**观察 key 全维度**（configID+规范化 provider+规范化 BaseURL+model+endpoint，v1.10 修复：配置只换上游时旧观察天然失效不被继承），TTL 12h 防单次误判永久降级；`applyCapabilityRouting` 在两个公开出口做声明化路由：json_object=unsupported 直接 free_text + markJSONModeDropped（审计如实），**temperature/max_tokens=unsupported 直接省略参数（v1.10 起最终 payload 不再无条件发送）**；运行时 4xx 识别三函数（`looksLikeUnsupportedJSONMode` 收紧为必须明确指向结构化字段、`looksLikeUnsupportedTemperature`/`looksLikeUnsupportedMaxTokens` 需字段指向+「不被接受」措辞，值超限不算）；四处回落点纪律（v1.10）：审计观测（markJSONModeDropped/markTemperatureOmitted/markMaxTokensOmitted）在重试前置位，**能力观察只在去参 fallback 请求确认成功后提交**（fallback 失败不污染能力状态）；max_tokens 兼容性省略=可观察的预算降级（观察+SysLog+审计 payload），非静默无上限。llm.go provider smoke：基础连通成功后追加最小 JSON 结构化探测（先无参 200 后带参 4xx 且文案指向结构化字段构成对照证据；网络与 5xx 非结论性不落观察），端点连通性同步落观察；探针 module=test 独立、不注业务 prompt，smoke supported≠业务可用（路由只消费 unsupported）。flag `llm_capability_routing` 缺省开、管理后台「LLM 准确性契约」卡第四开关；关闭仅回退声明化路由（观察记录与隐式回落不受控）。json_schema/function_calling/reasoning/tool 参数声明位与 P2-4 多模型路由待后续（**只定义了枚举字段，未参与决策，如实声明未实现**） |
