...
> 状态：方案基线（v1.12；**P0 全部九项已实施**——第五批 P0-6 prompt 不可变快照与 L0-L3 分层收官，v1.10 修复批收口独立审查确认的六类实现缺口，v1.11 加固上游 60s/504 长调用，v1.12 取消业务 prompt 输出篇幅限制；此前 P0-1 全部、P0-7 stale/PIT 与机读拒答码、P0-2 统一运行元数据、P0-8 调用关联/审计完整性、P0-3 证据链 ev4、P0-4 semantic validator、P0-5 capability matrix、P0-9 模块化输出预算均已实施）<br>
> 编制日期：2026-07-19；v1.4 修订：2026-07-20；v1.5 修订：2026-07-21（Asia/Shanghai）；v1.5 审查修复：2026-07-21（repair 漏标、完整性门禁、日报/新闻 PIT、拒答码端到端、取消审计脱敏）；v1.6：2026-07-21（P0-2/P0-8 实施）；v1.7：2026-07-21（P0-3/P0-4 实施）；v1.8：2026-07-22（P0-5/P0-9 实施）；v1.9：2026-07-22（P0-6 实施，P0 收官）；v1.10：2026-07-22（P0 修复批——独立审查确认缺口收口）；v1.11：2026-07-22（全 LLM 后台化后的上游 504 加固）；v1.12：2026-07-22（取消业务 prompt 输出篇幅限制）<br>
...
| 候选池内 LLM 精选与 coverage 诊断 | AlphaSift | 记录 unknown/duplicate/out-of-pool/prompt_trimmed 和 fallback | P1（P1-1，与 §7.2 一致；v1.10 修正此前误标 P0 的内部矛盾——反编造过滤/量化 fallback 已有，coverage 诊断字段属 P1-1） |
