...
| N2 | 新闻→AI 信号：情绪增强/分析注入/日报事件段/推荐因子 | ✅ 完成（`service/newsai.go` 分级情绪增强+聚合情绪分、个股分析/问答舆情段（p6/q4）、日报事件段 4 步硬规则（`newsevent.go`）、推荐消息面因子（p6/s4）+前端情绪标签；commit 见 git log） |
| F1 | 东财 datacenter 网关 + 财报日历/业绩预告 + 公告 + 提醒联动 | ☐ 未开始 |
