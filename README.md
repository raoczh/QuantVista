...
| 选股与回测 | 26 个内置选股策略、4 个参数模板、自定义条件树、自然语言生成策略、因子扫描、历史时点回测与执行约束模拟 |
...
内置策略包含均线金叉、放量突破后回踩、布林收口突破、55 日通道突破、KDJ 低位修复等常用形态。选股条件、推荐侧重与入场确认相互对应；财务质量比较使用已披露年报 ROE，最新报告同比单独保留，缺失数据不会冒充真实零值。

个股分析与推荐追踪共用买入区间、目标和止损的程序计算：相同策略、周期与数据时点可复现相同价位，突破延伸过大或扣费后风险收益不足时保持观察。AI 解释依据和风险，原始价位提案单独保留。历史结果不重算；持仓保护仍结合实际成本和已发生的仓位变化。实现与验证边界见 [价格、策略与通知复核](docs/PRICE_STRATEGY_NOTIFICATION_REVIEW.md)。
...
| `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT` | Web Push 配置，用于页面休眠或关闭后的后台投递；未配置时仅在页面继续运行期间接收 |
...
| [价格、策略与通知复核](docs/PRICE_STRATEGY_NOTIFICATION_REVIEW.md) | 双模块共同价位、26 个选股策略、财务口径、A 股规则和后台提醒的验收范围 |
