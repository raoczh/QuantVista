...
func TestAnalysisEvidenceBackfill(t *testing.T) {
	...
	usage, _ := svc.fillAnalysisTrust(context.Background(), 1, &model.LLMConfig{}, "", false, AnalyzeRequest{Verify: false}, snapshot, result, "", "")
...
func TestAnalysisReviewRejectCascade(t *testing.T) {
	...
	usage, _ := svc.fillAnalysisTrust(context.Background(), 1, cfg, "k", true, AnalyzeRequest{Verify: true}, snapshot, result, "", "")
