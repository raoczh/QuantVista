...
规则：
1. 价位必须锚定数据快照：买入区间、目标价、止损价须依据现价、MA5/MA10/MA20、区间高低点、涨跌停价等快照中的具体数值推导，禁止凭空给价、禁止使用你记忆中的历史价位。
...
// 返回本次消耗的 token。仅 stock+标准模式+非回溯（as_of 的历史现价上做「当下计划」既无意义
// 又易被误读为回测结论）。评级偏空与风险闸门 block 两种确定性拒绝不烧 token。
func (s *AnalysisService) attachTradePlan(ctx context.Context, userID int64, cfg *model.LLMConfig, apiKey string, allowPrivate bool, req AnalyzeRequest, snapshot map[string]any, result *AnalysisResult) chatUsage {
	...
	_, usage, _, callErr := s.callWithRepair(ctx, userID, "trade_plan", cfg, apiKey, allowPrivate, messages, parse, tradePlanRepairHint)
