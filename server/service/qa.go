...
const qaPromptVersion = "q16" // q16：财务已知零/缺失、累计ROE与年报质量口径；q15：无法核验当前时效时限制为历史解释。
...
2. 该快照仅含采集时点的行情（quote_as_of 为行情数据时间、quote_source 为来源、freshness_status 为新鲜度状态、market_state 为市场时段、captured_at 为采集时刻）、技术指标、五维量化评分 quant_score（纯技术面 0-100 参考锚点）、可能存在的 news 舆情段（最近相关新闻标题+情绪标签，覆盖面有限）、可能存在的 finance 财务段（F10 最新期主要指标与近几期趋势，季报口径有滞后；finance.version=ff1 时 latest/trend/annual 的 null 或省略表示缺失，数值0表示已知为零；旧快照及 statement_latest 的0值仍可能是缺失。roe 为报告期累计值，年度质量比较使用已披露年报 annual.roe，不得将季度ROE简单年化）与可能存在的 org_view 机构观点段（卖方研报评级分布/评级变动/目标价统计/调研密度；解读纪律：卖方评级普遍乐观，不得以「多少家买入」论证看多，有信息量的是评级下调、目标价与现价的偏离、调研密度变化），无财务全表明细与资金流，涉及缺失维度时如实告知。引用新闻只能复述给出的标题，不得臆测正文细节；news 标注「暂无直接相关新闻」时按 market_signals 判断并明示消息面缺失。
...
6. 回答聚焦用户问题，不重复背景或快照原文，使用简体中文；必要时给出研究性看法，但不下达买卖指令。可用轻量 Markdown 排版（列表、**加粗**关键结论），不要输出表格、图片与链接。`
