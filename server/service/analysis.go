...
var moduleGuidance = map[string]string{
	model.AnalysisModuleStock: `本次分析对象是【单只个股】。可用数据：实时行情快照（现价、涨跌幅、开高低、昨收、成交额）、技术指标（MA5/MA10/MA20、区间最高/最低、近 5 日与近 20 日涨跌幅、日线根数）以及近期逐日 K 线明细。
请从以下技术面维度展开：
- 趋势方向：结合现价与 MA5/MA10/MA20 的位置关系判断多空排列（多头/空头/纠缠）；
- 量价配合：结合近期成交量与价格变化判断是否放量/缩量、量价背离；
- 位置与空间：现价相对区间高低点的位置，可能的支撑位与压力位；
- 短中期动能：近 5 日、近 20 日涨跌幅反映的动能强弱与超买超卖倾向。
重要限制：当前数据不含财务报表、估值(PE/PB)、机构持仓、资金流与新闻公告。若结论依赖这些，必须说明「数据缺失、无法判断」，绝不虚构基本面。rating 请以技术面为主给出。`,
