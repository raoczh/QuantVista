...
type AnalysisResult struct {
	...
	Summary       string   `json:"summary"`       // 核心总览
...
func (s *AnalysisService) fillAnalysisTrust(ctx context.Context, userID int64, cfg *model.LLMConfig, apiKey string, allowPrivate bool, req AnalyzeRequest, snapshot map[string]any, result *AnalysisResult, traceID, parentRunID string) (chatUsage, *llmRun) {
	...
	// ev5（P1-2）：结论级 claims——总结（失效条件=kill_switches）与交易计划（失效条件=
	// 计划 invalidators）升格为可追踪声明；evidence_ids/status 程序推导，非模型自报。
	result.EvidenceCheck.Claims = deriveClaims(result.EvidenceCheck, analysisClaimSpecs(result))
...
只输出 JSON：{"verdict":"pass|warn|reject","comment":"中文说明","confidence":0-100 的复核后建议置信度整数}，不要任何解释或代码块标记。confidence 给具体数值（reject 时给你认为的真实低值）；填 0 表示维持原置信度不调整。
重点核查：亏损收窄是否被写成盈利成长、同源指标是否被写成独立验证、技术策略是否因长持有周期而变成财务论证、程序买卖价和保护条件是否被模型另造价位覆盖。` + "\n" + earningsPromptDiscipline
...
- anti_thesis: 字符串数组，反方观点——针对你给出的结论论证「为什么它可能是错的」，至少给出一条有实质内容的论据
...
只输出一个 JSON 对象，不要任何解释文字，也不要 Markdown 代码块标记。字段：
- roles: 数组，恰好 4 个元素，每个元素为 {"role": "technical"|"momentum"|"risk"|"contrarian", "rating": "bullish"|"neutral"|"bearish", "summary": "该角色核心观点"}
