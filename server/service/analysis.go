...
const analysisRepairHint = "请只输出一个合法 JSON 对象，严格包含字段 rating(bullish/neutral/bearish)、confidence(0-100 整数)、summary、highlights、risks、opportunities、suggestions、anti_thesis、kill_switches、unknowns、disclaimer；数组沿用条数与每条 60 字上限，不要任何解释或代码块标记。"

const panelRepairHint = "请只输出一个合法 JSON 对象，严格包含字段 roles（恰好 4 个元素的数组，每个元素含 role(technical/momentum/risk/contrarian)、rating(bullish/neutral/bearish)、summary 且不超过 60 字）、consensus、disagreement（均不超过 100 字），不要任何解释或代码块标记。"
	...
	addSec := func(module string, texts ...string) {
		for _, t := range texts {
			if strings.TrimSpace(t) != "" {
				sections = append(sections, evidenceSection{Module: module, Text: t})
			}
		}
	}
	addSec("总结", result.Summary)
	addSec("要点", result.Highlights...)
	addSec("风险", result.Risks...)
	addSec("机会", result.Opportunities...)
...
只输出 JSON：{"verdict":"pass|warn|reject","comment":"不超过 80 字的中文说明","confidence":0-100 的复核后建议置信度整数}，不要任何解释或代码块标记。confidence 给具体数值（reject 时给你认为的真实低值）；填 0 表示维持原置信度不调整。`
...
var moduleGuidance = map[string]string{
	model.AnalysisModuleStock: `本次分析对象是【单只个股】。可用数据：采集时点的行情快照（现价、涨跌幅、开高低、昨收、成交额；快照顶层 captured_at 为采集时刻、quote_as_of/quote_source 为行情数据时间与来源、bars_as_of 为日线截至日、market_state 为市场时段、freshness_status 为新鲜度状态）、估值与盘面快照 valuation（PE-TTM/动态 PE、市净率、总市值/流通市值、换手率、振幅、量比、涨停/跌停价；该块缺失时表示估值数据暂不可得）、技术指标（MA5/MA10/MA20、区间最高/最低、近 5 日与近 20 日涨跌幅、日线根数）、五维量化评分 quant_score（趋势/动量/位置/量能/风险加权 0-100，与本站个股评分页同口径）以及近期逐日 K 线明细。
请从以下维度展开：
...
- 情绪温度（若有 mood 块）：连板高度与分布反映情绪周期位置（高度压缩=退潮、梯队完整=发酵），炸板率高说明分歧加大，昨涨停溢价（yzt_avg_chg）为正且红盘比例高说明打板资金仍有赚钱效应；mood 数据为盘后快照，trade_date 标注归属日，若早于当日请注明是上一交易日情绪；
...
- highlights: 字符串数组，关键要点（2~4 条，每条不超过 60 字）
- risks: 字符串数组，主要风险（2~4 条，每条不超过 60 字）
- opportunities: 字符串数组，潜在机会（1~3 条，每条不超过 60 字）
- suggestions: 字符串数组，需关注的点或研究方向（通常 2~4 条，每条不超过 60 字；持仓分析按逐仓纪律覆盖快照内有有效行情的持仓时可超过 4 条）
- anti_thesis: 字符串数组，反方观点——针对你给出的结论论证「为什么它可能是错的」（1~2 条，每条不超过 60 字，须言之有物）
- kill_switches: 字符串数组，结论失效条件——出现什么可观察信号说明本次判断已不成立（1~3 条，每条不超过 60 字）
- unknowns: 字符串数组，数据盲区——本次数据看不到、但对结论重要的信息（1~3 条，每条不超过 60 字）
- disclaimer: 字符串，不超过 60 字的风险与免责提示
输出有严格时限，精炼比铺陈更重要；不得重复同一观点或额外增加字段。`
...
- consensus: 字符串，四个角色的共识结论（不超过 100 字）
- disagreement: 字符串，主要分歧点与取决条件（不超过 100 字）
输出有严格时限，不得复述四个角色原文或额外增加字段。`
