...
var llmRoleAssets = map[string]LLMRoleAsset{
	...
	"trade_plan": {
		...
		Purpose: "A股使用与推荐共用的程序价格计划并解释等待条件；旧路径及港美市场保留模型候选价与服务端硬校验",
		Market:  "cn 为主（随主分析：us/hk 可达但数据受限）", Horizons: "计划自带持有周期字段",
		Trigger:          "个股标准分析成功后附加（非 panel/非 as_of）；存在price_plan时零额外AI调用，评级偏空/风险闸门/报价不可用不给执行计划",
		...
		ForbiddenActions: []string{"改写共同price_plan或计算仓位/RR/费用（Go 回填）", "stale 行情下给精确价位", "把估值区间、风险目标和实际持仓保护混作同一个卖价"},
	...
	"news": {
		...
		Market:  "cn", Horizons: "[now-7d,now] 窗口（nw3 window_meta：完整窗口统计+已标注独立来源判定+查询状态）",
	...
	"reflection": {
		...
		Trigger:          "tracking job 标签结算后；全库当前版本的成熟标签（next_open）≥30 才启用、每轮 ≤5 条；影子检索 available_from ≤ 检索时点（防回放泄漏铁律）",
