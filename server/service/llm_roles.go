...
var llmRoleAssets = map[string]LLMRoleAsset{
	"analysis": {
		...
		Market:  "cn", Horizons: "标准分析无固定持有期；panel 为四视角短评",
	...
	"trade_plan": {
		...
		MustAnswer:       []string{"止损<现价、止损<买入区间下沿、目标>区间上沿（validateTradePlan 硬纪律）", "invalidators 计划失效条件（P1-2）", "操作清单 checklist"},
	...
	"news": {
		...
		Market:  "cn", Horizons: "[now-7d,now] 窗口（nw1 window_meta）",
	...
	"debate_bull": {
		...
		MustAnswer:       []string{"每条 claim 引用 evidence_id（越界剥除）", "至少一条失效条件（全无报错触发 repair）"},
	...
	"debate_bear": {
		...
		MustAnswer:       []string{"challenges 逐条回应 bull claim id（引用校验剥除非法引用）", "区分已证实风险与假设风险"},
	...
	"debate_judge": {
		...
		MustAnswer:       []string{"按证据质量排序不按角色票数平均（§6.3 反投票合成锚）", "block 时 verdict 不得 bullish（收口校验，repair 仍违纪 judge_invalid）", "冲突输出 conflict_note 不和稀泥"},
