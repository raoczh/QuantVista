...
func enhanceBatchLLM(ctx context.Context, cfg *model.LLMConfig, apiKey string, allowPrivate bool, rows []model.News, adminID int64) (map[int64]newsEnhanceItem, error) {
	type inRow struct {
		ID    int64  `json:"id"`
...
func (s *NewsService) EnhanceNewsRound(ctx context.Context) {
	...
	for i := 0; i < len(llmQueue); i += newsAIBatchSize {
		...
		results, err := enhanceBatchLLM(ctx, cfg, apiKey, allowPrivate, batch, adminID)
		for _, n := range batch {
			if err == nil {
