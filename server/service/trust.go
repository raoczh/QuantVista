...
// Unit 记规范化口径；AsOf 为该字段的数据时间（尽力而为）；Source 为该字段的数据源标识
// （尽力而为，ev4）；Derived 标记亿元衍生值；
...
type evidenceItem struct {
	...
	// EvidenceID 证据链 ID（ev4，P0-3）：命中项按顺序分配 ev-001…，与 Path/Source/AsOf/
	// SnapValue 一起构成 field_path/source/as_of/value 四元证据引用（§5.2 evidence_refs
	// 的程序化实现——由核验引擎从快照推导，非模型自报）。
	...
	Source     string  `json:"source,omitempty"`     // 命中字段的数据源标识（ev4，尽力而为）
...
// evidenceUnknown 结构化数据缺口（ev4，P0-3）：快照 builder 对「取不到的数据段」的
// 显式声明（field_path/reason/impact），随快照喂给模型（区分「没有数据」与「数据为零」）
// 并透出前端展示。与散落的 *_note 字符串不同，这是机器可读契约。
...
// evidenceKeySection 关键结论段的证据覆盖（ev4，P0-3）：模块的「关键结论」文本段
// （分析=总结、问答=回答、对比=AI点评、日报=总结）中可核验数字的快照佐证计数。
// SnapshotMatched=0 且 Total>0 表示关键结论没有任何数字被数据快照佐证。
type evidenceKeySection struct {
	Module          string `json:"module"`
	Total           int    `json:"total"`
	SnapshotMatched int    `json:"snapshot_matched"`
}
...
// **「浮亏/亏损/亏」与「浮盈/盈利/盈」是 D17 起补入的**：模型描述持仓盈亏时几乎必然写
// 「浮亏 20%」而不是「浮亏 -20%」，而快照里的 pnl_pct 是带符号的 -20——词表缺这几个词
// 会把**正确的复述判成幻觉**（同 B8「口径基数 10 必须进值域」的同类问题）。
// 加方向词只会让更多正确表述被匹配上，不会放行错误表述：跨符号匹配仍要求方向词与
// 快照符号一致（directionMatchesSign），把涨说成跌照样记 direction_mismatch。
// 长词必须排在其子串之前（"亏损" 在 "亏" 前），否则短词先命中会让长词永不生效。
var evidenceDownWords = []string{"下跌", "跌破", "回落", "回撤", "净流出", "流出", "下调", "浮亏", "亏损", "亏", "跌", "降"}
var evidenceUpWords = []string{"上涨", "涨至", "突破", "净流入", "流入", "上调", "浮盈", "盈利", "盈", "涨", "升"}
...
func verifyEvidenceLabeled(sections []evidenceSection, vals []labeledValue) *evidenceCheck {
	...
	// ev4：命中项按顺序分配证据链 ID（ev-001…），Path/Source/AsOf/SnapValue 构成
	// 可回指快照的 evidence_refs（程序化推导，非模型自报）。截断后分配保证 ID 与
	// 落库明细一一对应。
	evSeq := 0
	...
	for _, it := range items {
		if it.Matched {
			evSeq++
			it.EvidenceID = fmt.Sprintf("ev-%03d", evSeq)
		}
...
// markKeySection 标注关键结论段（ev4，P0-3）：统计指定模块段里可核验数字的快照佐证数，
// 供「关键结论至少 1 个快照佐证」的覆盖度量与前端警示。段内无可核验数字时 Total=0
// （关键结论不含数字本身不是错误，但覆盖指标如实为 0/0）。
func markKeySection(check *evidenceCheck, module string) {
	if check == nil {
		return
	}
	ks := &evidenceKeySection{Module: module}
	for _, it := range check.Items {
		if it.Module != module {
			continue
		}
		ks.Total++
		if it.Matched && it.Origin == "" {
			ks.SnapshotMatched++
		}
	}
	check.KeySection = ks
...
// hints：路径前缀 → 数据时间/数据源（"" 通配全部），命中前缀者填 AsOf/Source。
func snapshotLabeledValues(snapshot any, hints *snapshotHints, exclude ...string) []labeledValue {
	...
	hintFor := func(m map[string]string, path string) string {
		best := ""
		bestLen := -1
		for pre, v := range m {
			if strings.HasPrefix(path, pre) && len(pre) > bestLen {
				best, bestLen = v, len(pre)
			}
		}
		return best
	}
	asOfFor := func(path string) string {
		...
		return hintFor(hints.asOf, path)
	}
...
// stockFieldHints 从个股快照的元数据组装字段提示（ev4：as_of + source 双维；
// 旧快照无元数据时对应维为空）。source 维的结构性事实（technicals/quant_score 派生自
// 本地日线库、finance 来自东财 F10、org_view 来自东财 datacenter）不依赖逐次采集元数据，
...
func stockFieldHints(snap map[string]any) *snapshotHints {
	...
	if val, ok := snap["valuation"].(map[string]any); ok {
		if s, ok := val["source"].(string); ok && s != "" {
			h.source["valuation."] = s
		}
		if s, ok := val["source_data_time"].(string); ok && s != "" {
			h.asOf["valuation."] = s
		}
	}
	if _, ok := snap["technicals"]; ok {
		h.source["technicals."] = "daily_bars"
		h.source["quant_score."] = "daily_bars"
	}
...
// snapshotUnknownItems 从快照提取结构化数据缺口（ev4，P0-3）：快照 builder 在
// snap["unknowns"] 落的 field_path/reason/impact 数组。兼容 Go 原生 []map[string]any
// 与 JSON 反序列化后的 []any 两形态（问答复用落库快照）。旧快照无该键返回 nil。
func snapshotUnknownItems(snap map[string]any) []evidenceUnknown {
	raw, ok := snap["unknowns"]
	if !ok {
		return nil
	}
	var out []evidenceUnknown
	appendItem := func(m map[string]any) {
		fp, _ := m["field_path"].(string)
		if fp == "" {
			return
		}
		reason, _ := m["reason"].(string)
		impact, _ := m["impact"].(string)
		out = append(out, evidenceUnknown{FieldPath: fp, Reason: reason, Impact: impact})
	}
