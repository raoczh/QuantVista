...
func (s *TrackingService) evaluateOne(ctx context.Context, batch model.RecommendationBatch, rec model.Recommendation, recDate string, bench []datasource.Bar, pos *model.Position) *model.RecommendationStatus {
	...
	// 否则收益/触发判定整体错位（旧行为只能在 note 里认错）。
	refPrice, takeProfit, stopLoss := rec.RefPrice, detail.TakeProfit, detail.StopLoss
	rebased := false
	if rec.RefDate != "" && rec.RefClose > 0 && refAnchorClose > 0 && relDiff(refAnchorClose, rec.RefClose) > rebaseTolerance {
		factor := refAnchorClose / rec.RefClose
		refPrice = round2(refPrice * factor)
		takeProfit = round2(takeProfit * factor)
		stopLoss = round2(stopLoss * factor)
		st.RefPrice = refPrice
		rebased = true
	}
	...
	if res.Outcome == model.RecOutcomeNoData {
		...
		notes := []string{}
		if rebased {
			notes = append(notes, "检测到前复权重锚，参考价与计划价已按复权因子调整")
		} else {
			notes = append(notes, nonAdjustedNote)
		}
		if !res.HasBench {
...
//
// S0-4 口径铁律：**买入胜率只统计 action=buy AND 已成熟**（短线=止盈/止损/过期终态；
// 长线=已过 60 交易日复盘周期）；watch 单独统计「观察判断质量」；active/pending 不进
// 胜率分母；degraded 批次条目单独计数不混入。旧的全样本字段保留（趋势参考），前端
// 以买入成熟口径为主展示。
type PerformanceStats struct {
	Type              string  `json:"type"`   // "" 全部 / short_term / long_term
	Sample            int     `json:"sample"` // 样本量 n（有价格数据的条目，含未成熟；趋势参考）
	...
	WatchWinRate float64 `json:"watch_win_rate"`

	// degraded 批次条目（量化降级/拒选批次）单独计数，不混入以上统计。
...
// statusMatured 追踪状态是否已成熟（S0-4 买入胜率口径）：短线=价格/时间终态；
// 长线无价格触发概念，按超过复盘周期视为到期成熟。
func statusMatured(r model.RecommendationStatus) bool {
	switch r.Outcome {
	case model.RecOutcomeTakeProfit, model.RecOutcomeStopLoss, model.RecOutcomeExpired:
		return true
	case model.RecOutcomeTracking:
		return r.ElapsedTradeDays >= longRecReviewDays
	}
	return false
}
...
func (s *TrackingService) Performance(userID int64, recType string) (*PerformanceStats, error) {
	...
	for _, r := range rows {
		...
		if degradedBatch[r.BatchID] {
			stats.DegradedExcluded++
			continue
		}
		stats.Sample++
		sumRet += r.ReturnPct
		...
		benchOK := !strings.Contains(r.Note, benchUnavailableNote)
		if benchOK { // 基准有效才计入 alpha 样本
			stats.BenchSample++
			sumAlpha += r.AlphaPct
		}
		// 买入成熟口径（主指标）与观察口径分开。
		matured := statusMatured(r)
		if r.Action == model.RecActionBuy {
			if matured {
				...
				if benchOK {
					stats.BuyBenchSample++
					buySumAlpha += r.AlphaPct
				}
			} else {
				stats.BuyActive++
			}
		} else if r.Action == model.RecActionWatch {
			stats.WatchSample++
			if r.ReturnPct > 0 {
				watchWins++
			}
	...
	if stats.BuyBenchSample > 0 {
		stats.BuyAvgAlphaPct = round2(buySumAlpha / float64(stats.BuyBenchSample))
	}
	if stats.WatchSample > 0 {
		stats.WatchWinRate = round2(float64(watchWins) / float64(stats.WatchSample) * 100)
	}
	if stats.Sample7d > 0 {
...
func StartTrackingJobs(mgr *datasource.Manager) {
	...
	refreshAll := func() {
		...
		// S0 标签事实推进（幂等）：统一执行模拟器结算已走完持有期的标签 + 补建
		// 实际建仓（血缘）标签行。日线来自本地 daily_bars，零上游成本。
		lctx, lcancel := context.WithTimeout(context.Background(), 10*time.Minute)
		if _, err := AdvanceRecommendationLabels(lctx, svc.market); err != nil {
