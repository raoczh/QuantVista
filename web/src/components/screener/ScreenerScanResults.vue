...
const selectableSymbols = computed(() => displayedSymbols.value.slice(0, 100))
const allSelected = computed(() => selectableSymbols.value.length > 0 && selectableSymbols.value.every((s) => props.selectedSymbols.includes(s)))
const someSelected = computed(() => !allSelected.value && displayedSymbols.value.some((s) => props.selectedSymbols.includes(s)))
const dataState = computed(() => {
  if (props.result.lag_open_days == null || props.result.lag_open_days < 0) return { label: '数据时效未知', type: 'warning' as const }
  if (props.result.lag_open_days > 0) return { label: `数据落后 ${props.result.lag_open_days} 个交易日`, type: 'warning' as const }
  return { label: '扫描时日线已更新', type: 'info' as const }
})
...
function toggleAll(checked: boolean) {
  if (props.scanning || props.batchLoading) return
  emit('update:selectedSymbols', checked ? selectableSymbols.value : [])
}
  ...
  <SectionCard :title="`扫描结果 · ${result.strategy}`" class="block">
    <template #extra>
      <div class="result-switches">
        <label><n-switch :value="includeST" :disabled="scanning || batchLoading" size="small" @update:value="updateIncludeST" /> 含 ST</label>
        <label><n-switch :value="includeStale" :disabled="scanning || batchLoading" size="small" @update:value="updateIncludeStale" /> 含停牌或滞后</label>
      </div>
    </template>
    <p class="result-meta">
      {{ stats }}
    ...
    <n-alert v-if="result.stale_note" type="warning" :bordered="false" class="result-warning">{{ result.stale_note }}</n-alert>
    <n-alert v-if="resultIncludesStale" type="warning" :bordered="false" class="result-warning">
      下方快照包含停牌或滞后数据参与的扫描，命中项不能视为正常实时结果；请进入个股详情核对行情时间。
    </n-alert>
    ...
    <div v-if="result.items?.length" class="batch-toolbar">
      <div>
        <n-checkbox :checked="allSelected" :indeterminate="someSelected" :disabled="scanning || batchLoading" @update:checked="toggleAll">{{ displayedSymbols.length > 100 ? '选择前 100 只' : '全选当前结果' }}</n-checkbox>
        <span>已选 {{ selectedSymbols.length }} / 100</span>
      </div>
      <div>
        <n-button v-if="selectedSymbols.length" size="small" quaternary :disabled="scanning || batchLoading" @click="emit('update:selectedSymbols', [])">清空</n-button>
        <n-button size="small" type="primary" :disabled="!selectedSymbols.length || scanning" :loading="batchLoading" @click="emit('batch')">
          批量加入自选
        </n-button>
      </div>
    </div>
    ...
    <div v-else class="qv-scroll-x">
      <n-table size="small" :single-line="false" class="hits-table">
        <thead><tr><th>选择</th><th>股票</th><th class="num">收盘价</th><th class="num">涨跌</th><th class="num">成交额(亿)</th><th v-if="!isMobile" class="num">换手%</th><th v-if="!isMobile" class="num">60日位置</th><th>命中原因与风险</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="hit in result.items" :key="hit.symbol">
            <td><n-checkbox :checked="selectedSymbols.includes(hit.symbol)" :disabled="scanning || batchLoading || (!selectedSymbols.includes(hit.symbol) && selectedSymbols.length >= 100)" :aria-label="`选择 ${hit.name || '名称待补全'}`" @update:checked="toggleSymbol(hit.symbol, $event)" /></td>
            <td><StockIdentity :symbol="hit.symbol" market="cn" :name="hit.name" :density="isMobile ? 'normal' : 'table'" clickable actions /></td>
            ...
            <td v-if="!isMobile" class="num qv-tnum">{{ hit.turnover_rate != null ? hit.turnover_rate.toFixed(2) : '未知' }}</td>
            <td v-if="!isMobile" class="num qv-tnum">{{ hit.pos_60 != null ? `${hit.pos_60.toFixed(0)}%` : '未知' }}</td>
            <td>
...
.hits-table { min-width: 920px; }
.hits-table th:nth-child(2), .hits-table td:nth-child(2) { min-width: 180px; }
@media (max-width: 768px) {
  .hits-table { min-width: 740px; }
